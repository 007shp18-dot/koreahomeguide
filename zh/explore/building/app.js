...
function renderAreaGroups(items) {
  ...
  buildingAreaGroups.innerHTML = items.flatMap(group => {
    ...
    return [`<div class="size-band-card market-evidence-row"><span>约 ${group.approxAreaSqm}㎡</span><strong class="market-evidence-rent">${rent}</strong><small class="market-evidence-count">${evidence.sampleLabel}${deposit}</small></div>`];
  }).join('');
}
...
function renderBuilding(data) {
  ...
  buildingChange.textContent = Number.isFinite(change) ? `原始月租中位数变化：${change > 0 ? '+' : ''}${change.toFixed(1)}% · 包含押金、面积和合同构成变化` : '原始趋势：数据不足';
