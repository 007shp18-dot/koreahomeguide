...
function ensureResultFeedback(){let confidenceDetails=document.querySelector('#rentCheckConfidenceDetails');if(!confidenceDetails){confidenceDetails=document.createElement('details');confidenceDetails.id='rentCheckConfidenceDetails';confidenceDetails.className='rent-check-confidence-details';confidenceDetails.hidden=true;confidenceDetails.innerHTML=`<summary>${zh?'为什么是这个可信度？':'Why this confidence?'}</summary><p id="rentCheckConfidenceExplanation"></p>`;result.querySelector('.rent-check-verdict').appendChild(confidenceDetails);}let nextStep=document.querySelector('#rentCheckNextStep');if(!nextStep){nextStep=document.createElement('section');nextStep.id='rentCheckNextStep';nextStep.className='rent-check-next-step';nextStep.hidden=true;nextStep.innerHTML=`<span class="eyebrow">${zh?'下一步':'NEXT STEP'}</span><h3 id="rentCheckNextHeading"></h3><p id="rentCheckNextBody"></p><div class="rent-check-next-actions"><a id="rentCheckNextPrimary" class="search-button" href="#"></a><a id="rentCheckNextSecondary" class="rent-check-secondary-action" href="#"></a></div>`;}const summaryPanel=result.querySelector('.rent-check-summary');summaryPanel.insertAdjacentElement('afterend',nextStep);return{confidenceDetails,confidenceExplanation:confidenceDetails.querySelector('#rentCheckConfidenceExplanation'),nextStep,nextHeading:nextStep.querySelector('#rentCheckNextHeading'),nextBody:nextStep.querySelector('#rentCheckNextBody'),nextPrimary:nextStep.querySelector('#rentCheckNextPrimary'),nextSecondary:nextStep.querySelector('#rentCheckNextSecondary')};}
...
const nextSecondary=feedback.nextSecondary;
...
function renderConfidence(data,mapped){const explanation=KHGRentCheckUI.confidenceExplanation(data,mapped.isStudioMapped);confidenceDetails.hidden=!explanation;confidenceExplanation.textContent=explanation;if(explanation)confidenceDetails.open=false;}
function setNextAction(anchor,action,exploreHref){anchor.textContent=action.label;anchor.href=action.href==='explore'?exploreHref:action.href;anchor.dataset.actionId=action.id;}
function renderNextStep(data,mapped){const config=KHGRentCheckUI.resultNextStep(data.rating);const exploreHref=KHGRentCheckUI.explorerUrl(area.value,mapped.officialType,language);nextHeading.textContent=config.heading;nextBody.textContent=config.body;setNextAction(nextPrimary,config.primary,exploreHref);setNextAction(nextSecondary,config.secondary,exploreHref);nextStep.hidden=false;latestResultContext={mapped,data};}
function trackNextAction(anchor){if(!latestResultContext)return;const context=trackBase(latestResultContext.mapped,latestResultContext.data);if(context)safeTrack('rent_check_next_action',{...context,action_id:String(anchor.dataset.actionId||'')});}
function renderResult(data,mapped){result.hidden=false;rating.textContent=KHGRentCheckUI.ratingLabel(data.rating);rating.className=`rent-rating ${data.rating||'insufficient'}`;message.textContent=KHGRentCheckUI.resultSentence(data);asking.innerHTML=moneyHtml(data.askingValueWon);median.innerHTML=data.medianValueWon==null?'-':moneyHtml(data.medianValueWon);difference.textContent=data.differencePct==null?'-':KHGRentCheckUI.formatDifference(data.differencePct);if(data.confidence){confidence.hidden=false;confidence.textContent=KHGRentCheckUI.confidenceLabel(data.confidence);confidence.className=`confidence-pill ${data.confidence}`;}else confidence.hidden=true;const count=Number(data.comparableCount||0),months=Number(data.monthsUsed||12);meta.textContent=data.rating==='insufficient'?(zh?`已搜索最近 ${months} 个完整月份，但数据仍不足以做出可靠判断。`:`Searched the latest ${months} completed months; not enough data for a reliable verdict.`):(zh?`${count} 笔可比成交 · 最近 ${months} 个完整月份。`:`${count} comparable contracts · latest ${months} completed months.`);summary.textContent=data.rating==='insufficient'?(zh?`找到 ${count} 笔可能匹配的成交；至少需要 3 笔合适记录才能判断。`:`${count} possible matches found; at least 3 suitable records are required.`):(zh?`${count} 笔已签约成交符合当前比较条件。`:`${count} signed contracts matched the current comparison.`);renderConfidence(data,mapped);renderDistribution(data);renderRows(data.comparables||[]);renderNextStep(data,mapped);}

if(form)form.addEventListener('submit',async e=>{e.preventDefault();syncCurrencyInput(deposit);syncCurrencyInput(rent);const depositWon=getInputWon(deposit),rentWon=getInputWon(rent),sqm=Number(areaSqm.value);if(!Number.isFinite(sqm)||sqm<=0)return setStatus(zh?'面积必须大于 0。':'Size must be greater than zero.','error');const mapped=KHGRentCheckUI.mapRentCheckType(type.value);updateStudio();setStatus(zh?'正在查找类似的官方成交记录…':'Finding similar official contracts…','loading');button.disabled=true;safeTrack('rent_check_start',trackBase(mapped,null));try{const q=new URLSearchParams({lawdCd:area.value,type:mapped.officialType,deposit:String(Math.round(depositWon)),rent:String(Math.round(rentWon)),area:String(sqm)});const r=await fetch(`/api/rent-check?${q}`);const data=await r.json();if(!r.ok)throw new Error(data.error||(zh?'租金比较失败。':'Rent comparison failed.'));renderResult(data,mapped);emitResult(data,mapped,depositWon,rentWon,sqm);setStatus(data.rating==='insufficient'?(zh?'相似的官方成交记录太少，因此没有给出价格判断。':'Too few similar official contracts for a price verdict.'):(zh?'比较完成。':'Comparison complete.'),'success');}catch(err){const context=trackBase(mapped,null);if(context)safeTrack('rent_check_error',{...context,error_category:errorCategory(err)});result.hidden=true;setStatus(KHGRentCheckUI.humanizeRentCheckError(err.message),'error');}finally{button.disabled=false;}});
if(type)type.addEventListener('change',updateStudio);
if(nextPrimary)nextPrimary.addEventListener('click',()=>trackNextAction(nextPrimary));
if(nextSecondary)nextSecondary.addEventListener('click',()=>trackNextAction(nextSecondary));
