const { median, percentile, completedMonthKeys, normalizeTransaction } = require('./rent-check-core.cjs');
...
function parseLeaseEnd(term) {
  const match = /^(\d{2})\.(\d{2})~(\d{2})\.(\d{2})$/.exec(String(term || '').trim());
  if (!match) return null;
  const month = Number(match[4]);
  if (month < 1 || month > 12) return null;
  return { year:2000 + Number(match[3]), month };
}

function renewalDelta(contract) {
  if (!contract || contract.contractType !== 'renewal') return null;
  const required = [contract.preDepositWon, contract.depositWon, contract.preMonthlyRentWon, contract.monthlyRentWon];
  if (required.some(value => value == null || String(value).trim() === '')) return null;
  const previousDeposit = Number(contract.preDepositWon);
  const currentDeposit = Number(contract.depositWon);
  const previousRent = Number(contract.preMonthlyRentWon);
  const currentRent = Number(contract.monthlyRentWon);
  if (![previousDeposit, currentDeposit, previousRent, currentRent].every(Number.isFinite)) return null;
  if (previousDeposit !== currentDeposit || previousRent === 0 || currentRent === 0) return null;
  return (currentRent - previousRent) / previousRent;
}
...
function observedFieldStats(rows) {
  const source = Array.isArray(rows) ? rows : [];
  const buildYears = source
    .filter(row => row && row.buildYear != null && row.buildYear !== '')
    .map(row => Number(row.buildYear))
    .filter(Number.isFinite);
  const floors = source
    .filter(row => row && row.floor != null && row.floor !== '')
    .map(row => Number(row.floor))
    .filter(Number.isFinite);
  const leaseEndHistogram = {};
  for (const row of source) {
    const end = parseLeaseEnd(row && row.contractTerm);
    if (!end) continue;
    const key = `${end.year}-${String(end.month).padStart(2, '0')}`;
    leaseEndHistogram[key] = (leaseEndHistogram[key] || 0) + 1;
  }
  const deltas = source.map(renewalDelta).filter(Number.isFinite);
  const pctValues = deltas.map(delta => delta * 100);
  return {
    buildYearMin:buildYears.length ? Math.min(...buildYears) : null,
    buildYearMax:buildYears.length ? Math.max(...buildYears) : null,
    buildYearMedian:median(buildYears),
    floorMin:floors.length ? Math.min(...floors) : null,
    floorMax:floors.length ? Math.max(...floors) : null,
    leaseEndHistogram,
    renewalDeltas:{
      count:deltas.length,
      medianPct:roundedPct(median(pctValues)),
      p25Pct:roundedPct(percentile(pctValues, 0.25)),
      p75Pct:roundedPct(percentile(pctValues, 0.75)),
      zeroCount:deltas.filter(delta => Math.abs(delta) < 1e-9).length,
      overCapCount:deltas.filter(delta => delta > 0.05 + 1e-9).length
    }
  };
}
