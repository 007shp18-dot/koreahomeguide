...
const { isRentCheckAreaCode, isSupportedPropertyType } = require('../providers/seoul-config.cjs');
...
function boundedAttributionString(value, max) {
  return String(value || '')
    .replace(/[\u0000-\u001f\u007f]/g, '')
    .trim()
    .slice(0, max);
...
function sanitizeAttribution(body = {}) {
  return {
    sourcePage:validatedResultSource(boundedAttributionString(body.sourcePage, 200)),
    utmSource:boundedAttributionString(body.utmSource, 120),
    utmMedium:boundedAttributionString(body.utmMedium, 120),
    utmCampaign:boundedAttributionString(body.utmCampaign, 120),
    referrerHost:boundedAttributionString(body.referrerHost, 255)
  };
}
...
function normalizeLeadPayload(body = {}, now = new Date()) {
  ...
  const privacyNoticeVersion = boundedString(body.privacyNoticeVersion, 20);
