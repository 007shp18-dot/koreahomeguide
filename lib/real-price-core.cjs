...
async function fetchRentalMonth({ serviceKey, type, lawdCd, dealYmd, fetchImpl = fetch, pageSize = 1000 }) {
  ...
  const cacheKey = `rent:${type}:${lawdCd}:${dealYmd}:${pageSize}`;
  return cachedMonthRequest(fetchImpl, cacheKey, () => fetchPagedXml({
    endpoint,
    serviceKey,
    lawdCd,
    dealYmd,
...
async function fetchSaleMonth({ serviceKey, type = 'apartment', lawdCd, dealYmd, fetchImpl = fetch, pageSize = 1000 }) {
  ...
  const cacheKey = `sale:${type}:${lawdCd}:${dealYmd}:${pageSize}`;
  return cachedMonthRequest(fetchImpl, cacheKey, () => fetchPagedXml({
    endpoint,
    serviceKey,
    lawdCd,
