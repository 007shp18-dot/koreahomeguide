const { fetchWithTimeout, DEFAULT_UPSTREAM_TIMEOUT_MS } = require('./api-guard.cjs');
...
async function fetchPagedXml({ endpoint, serviceKey, lawdCd, dealYmd, fetchImpl = fetch, pageSize = 1000, parser }) {
  async function fetchPage(pageNo) {
    ...
    const upstream = await fetchWithTimeout(fetchImpl, `${endpoint}?${params.toString()}`, {
...
async function fetchRentalMonth({ serviceKey, type, lawdCd, dealYmd, fetchImpl = fetch, pageSize = 1000 }) {
  ...
  return fetchPagedXml({ endpoint, serviceKey, lawdCd, dealYmd, fetchImpl, pageSize, parser: xml => parseItems(xml, type) });
}

async function fetchSaleMonth({ serviceKey, type = 'apartment', lawdCd, dealYmd, fetchImpl = fetch, pageSize = 1000 }) {
  const endpoint = saleEndpointForType(type);
  if (!endpoint) throw new TypeError('Unsupported sale property type.');
  return fetchPagedXml({ endpoint, serviceKey, lawdCd, dealYmd, fetchImpl, pageSize, parser: xml => parseSaleItems(xml, type) });
}
