...
function monthlyRentAtDeposit(monthlyRentWon, contractDepositWon, targetDepositWon, annualRate = DEPOSIT_CONVERSION_REFERENCE.annualRate) {
  return Number(monthlyRentWon) + ((Number(contractDepositWon) - Number(targetDepositWon)) * Number(annualRate) / 12);
}
...
function normalizeTransaction(item) {
  ...
  return {
    ...
    buildYear: item && item.buildYear != null && item.buildYear !== '' && Number.isFinite(Number(item.buildYear)) ? Number(item.buildYear) : null,
    floor: item && item.floor != null && item.floor !== '' && Number.isFinite(Number(item.floor)) ? Number(item.floor) : null,
    sggCd: String((item && item.sggCd) || '').trim(),
    umdCd: String((item && item.umdCd) || '').trim()
  };
}
...
function comparablesForTier(items, quote, tierConfig) {
  ...
  const filtered = items
    ...
    .filter(item => mode !== 'monthly-rent' || monthlyRentAtDeposit(item.monthlyRentWon, item.depositWon, depositWon) > 0);
...
function rateTypicalRange(askingValueWon, p25ValueWon, p75ValueWon) {
  if (askingValueWon < p25ValueWon) return 'below';
  if (askingValueWon > p75ValueWon) return 'above';
  return 'fair';
}
...
function resultFromComparables(comparables, quote, config) {
  ...
  const comparisonBasis = comparisonMode === 'monthly-rent' ? 'deposit-adjusted-monthly-rent' : 'jeonse-deposit';
  const conversionAnnualRate = comparisonMode === 'monthly-rent' ? DEPOSIT_CONVERSION_REFERENCE.annualRate : null;
  ...
  const values = comparables.map(item => comparisonMode === 'monthly-rent'
    ? monthlyRentAtDeposit(item.monthlyRentWon, item.depositWon, depositWon, conversionAnnualRate)
    : item.depositWon);
  ...
  const minValueWon = hasDistribution ? Math.min(...values) : null;
  const p25ValueWon = hasDistribution ? percentile(values, 0.25) : null;
  const p75ValueWon = hasDistribution ? percentile(values, 0.75) : null;
  const maxValueWon = hasDistribution ? Math.max(...values) : null;
  return {
    rating: hasDistribution
      ? rateTypicalRange(askingValueWon, p25ValueWon, p75ValueWon)
      : rateDifference(differencePct),
    comparisonMode,
    comparisonBasis,
    conversionAnnualRate,
    differencePct,
