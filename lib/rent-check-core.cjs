...
function percentile(values, p) {
  ...
  const position = (clean.length - 1) * bounded;
...
function percentileRank(values, askingValue) {
  const clean = values.map(Number).filter(Number.isFinite);
  const asking = Number(askingValue);
  if (!clean.length || !Number.isFinite(asking)) return null;
  const atOrBelow = clean.filter(value => value <= asking).length;
  return Math.max(0, Math.min(100, Math.round((atOrBelow / clean.length) * 100)));
}
...
function resultFromComparables(comparables, quote, config) {
  ...
  const p25ValueWon = percentile(values, 0.25);
  const p75ValueWon = percentile(values, 0.75);
  const quotePercentileRank = percentileRank(values, askingValueWon);
  ...
  return {
    ...
    p25ValueWon,
    p75ValueWon,
    percentileRank: quotePercentileRank,
