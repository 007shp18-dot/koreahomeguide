...
test('thread shows team replies and requires login to answer',async({page})=>{
 ...
 await page.goto(`/ko/community/${id}/`);await expect(page.getByRole('heading',{name:question.title})).toBeVisible();await expect(page.getByText('운영자 답변 내용입니다.',{exact:true})).toBeVisible();await page.getByRole('textbox',{name:'답변 작성',exact:true}).fill('추가로 궁금한 내용을 질문합니다.');await page.getByRole('button',{name:'답변 등록',exact:true}).click();await expect(page.getByLabel('아이디',{exact:true})).toBeVisible();await expect(page.getByRole('textbox',{name:'답변 작성',exact:true})).toHaveValue('추가로 궁금한 내용을 질문합니다.');
 expect(await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(0);
});
