...
test('questions retain the draft on failure and show held state without a fake public answer',async({page})=>{
 ...
 await page.goto('/ko/community/?market=seoul');await expect(page.getByRole('heading',{name:'집과 동네 질문'})).toBeVisible();await expect(page.getByRole('button',{name:'서울',exact:true})).toHaveAttribute('aria-pressed','true');
 ...
 await page.getByRole('button',{name:'질문 등록',exact:true}).click();await expect(page.getByRole('alert')).toContainText('입력한 내용은 유지했어요');await expect(page.getByLabel('내용',{exact:true})).toHaveValue(question.body);
 await page.getByRole('button',{name:'질문 등록',exact:true}).click();await expect(page.getByRole('status')).toContainText('아직 공개되지 않았어요');await expect(page.getByText('검토 대기',{exact:true})).toBeVisible();await expect(page.getByRole('link',{name:question.title})).toHaveAttribute('href',`/ko/community/${id}/`);
...
test('thread shows team replies and requires login to answer',async({page})=>{
 ...
 await page.goto(`/ko/community/${id}/`);await expect(page.getByRole('heading',{name:question.title})).toBeVisible();await expect(page.getByText('운영자 답변 내용입니다.',{exact:true})).toBeVisible();await page.getByLabel('답변 작성',{exact:true}).fill('추가로 궁금한 내용을 질문합니다.');await page.getByRole('button',{name:'답변 등록',exact:true}).click();await expect(page.getByLabel('아이디',{exact:true})).toBeVisible();await expect(page.getByLabel('답변 작성',{exact:true})).toHaveValue('추가로 궁금한 내용을 질문합니다.');
 expect(await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(0);
});
