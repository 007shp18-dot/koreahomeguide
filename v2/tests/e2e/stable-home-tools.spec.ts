...
test('home presents four city destinations and a separate budget journey without overflow', async ({page}) => {
 await page.goto('/');
 await page.evaluate(() => document.fonts.ready);
 await expect(page.locator('main [data-home-region="markets"]')).toHaveCount(1);
 await expect(page.locator('main [data-home-region="daily"] article')).toHaveCount(4);
 await expect(page.locator('main [data-home-region="daily"] time')).toHaveCount(4);
