...
test('Contract Check stays ordered, touch-sized, and keyboard reachable', async ({ page }) => {
  ...
  await expect(productNavigation.locator('.site-header__product-link')).toHaveText(['Explore', 'Buying & renting', 'Insights', 'Rankings', 'News', 'Tools', 'Community']);
  ...
  await expect(productNavigation.getByRole('link', { name: 'Buying & renting' }))
