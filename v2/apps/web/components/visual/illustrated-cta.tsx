...
export function IllustratedCta({ locale }: { locale: SiteLocale }) {
  ...
  return <section className={styles.cta} data-illustrated-cta="true" aria-label={copy[1]}>
    <div><p className={styles.ctaEyebrow}>SIGNEDPRICE / YOUR NEXT MOVE</p><h2>{copy[0]}<br/>{copy[1]}</h2><p>{copy[2]}</p><nav aria-label={locale === 'ko' ? '조금 더 자세히' : locale === 'zh-CN' ? '进一步了解' : 'Take a closer look'}><Link href={`${prefix}/prices/`}>{copy[3]} <span aria-hidden="true">→</span></Link><Link href={`${prefix}/guides/`}>{copy[4]}</Link><Link className={styles.ctaText} href={`${prefix}/tools/`}>{locale === 'ko' ? '예산 비교·비용 계산' : locale === 'zh-CN' ? '预算与费用工具' : 'Tools & calculators'}</Link></nav></div>
