...
export function KoreanMarketOverview({ market, facts, period, available }: Readonly<{ market: 'singapore' | 'dubai'; facts: readonly { label: string; value: string }[]; period: string; available: boolean }>) {
  ...
  return <KoreanSiteFrame href={sg ? '/ko/sg/' : '/ko/ae/dubai/'}><main>
    ...
    <section className={styles.section}><h2>이어서 살펴보기</h2><p>아래 서비스는 현재 영어로 제공됩니다.</p><div className={styles.productGrid}>
      ...
      <Link href={sg ? '/guides/read-singapore-private-transactions/' : '/ae/dubai/guide/'}><h3>구매 전 확인사항 (영문)</h3><p>가격을 비교하는 방법과 구입 전에 확인할 비용·서류를 정리했습니다.</p></Link>
