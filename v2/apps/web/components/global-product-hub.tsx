...
import { GuideDirectory, type GuideMarket } from './guides/guide-directory';
...
export function GlobalProductHub({ kind, newsWorkspace, guideMarket }: GlobalProductHubProps) {
  ...
  if (kind === 'guides') return <div id="top"><SiteHeader copy={{ ...headerFor(kind), links: [{ label: 'Guides', href: `/guides/${!guideMarket || guideMarket === 'all' ? '' : `?market=${guideMarket}`}`, isCurrent: true }] }} /><GuideDirectory market={guideMarket} /><SiteFooter copy={homepageCopy.footer} /></div>;
  return (
    <div id="top">
