...
export function DubaiOverview() {
  ...
  return <main>
    <MarketHero model={{ sectionLabel: 'Dubai market overview', eyebrow: 'Dubai market', heading: 'Dubai Market Overview', description: 'Compare Ready and Off-Plan prices, annual rents and estimated gross yields by area. Start with your budget, then narrow down the areas worth a closer look.', facts: [], layout: 'overview', tier: { state: 'limited', label: 'Area comparisons' } }} media={<MarketRepresentativePhoto photo={MARKET_PHOTOS.dubai} cityLabel="Dubai" eager />} />
    <div className={styles.main}>
    <section className={styles.section} aria-label="Dubai released evidence">
      <dl className={styles.metrics}><div><dt>Areas covered</dt><dd>{repository === null ? 'Unavailable' : areas.length}</dd></div><div><dt>Price comparison</dt><dd>Ready · Off-Plan</dd></div><div><dt>Rental comparison</dt><dd>Annual rent · Yield</dd></div></dl>
      <p>{context === undefined ? 'Area price data is temporarily unavailable.' : `Comparison period: ${context.comparisonPeriod.from}–${context.comparisonPeriod.to}. Data as of ${context.asOfDate}. Each area shows its own sample sizes; insufficient samples are withheld.`}</p>
