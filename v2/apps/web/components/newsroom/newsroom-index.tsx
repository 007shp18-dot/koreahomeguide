...
export function NewsroomIndex({ articles, policies, filters, headlines, locale = 'en' }: Readonly<{
  ...
  return <main className={styles.index} data-newsroom-layout="research" lang={locale}>
    ...
    {fullJourney && <>
      ...
      <section className={styles.section} id="city-journey"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{story.name[locale]}</p><h2>{ko ? '마음이 가는 도시, 내 집이 되기까지' : 'Your path to owning here'}</h2></div></div><JourneySteps key={story.city} stages={journeyStages} locale={locale} /></section>
      <section className={styles.section} id="find-your-place"><div className={styles.sectionHeading}><h2>{ko ? '나에게 맞는 동네와 집' : 'Find your place'}</h2><Link href={storyLinkHref(story.sections[3].links[0]?.href ?? cityStoryHref(story.city, locale), locale)}>{ko ? '동네 둘러보기' : 'Explore neighbourhoods'} <UiIcon name="arrow-right" /></Link></div>{comparisons.length ? <ArticleRows items={comparisons} /> : <div className={styles.simpleStory}><h3><Link href={journeyArticleHref(story.city, 'where', locale)}>{story.sections[3].title[locale]}</Link></h3><p>{story.sections[3].paragraphs[locale][0]}</p></div>}</section>
      <section className={styles.section} id="local-conversation"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{ko ? '현지의 질문을 더 깊이 읽기' : 'Questions people are asking locally'}</p><h2>{ko ? '현지에서 이야기하는 것들' : 'The local conversation'}</h2></div></div><ArticleRows items={pilotItems} /><p className={styles.eyebrow}>{ko ? '함께 살펴볼 현지의 질문' : 'More questions from the local conversation'}</p><div className={styles.issues}>{conversations.map((item, index) => <details key={item.url} open={index === 0} className={styles.issue}><summary><span>{CITY_STORIES.find(city => city.city === item.city)!.name[locale]}</span><h3>{item.title[locale]}</h3></summary><div><p>{item.point[locale]}</p><p><strong>{ko ? '내 집을 찾는다면' : 'For your home search'}</strong> · {item.check[locale]}</p><a href={item.url} target="_blank" rel="noopener noreferrer">{ko ? 'Reddit 원문 토론' : 'Read the Reddit discussion'} <UiIcon name="arrow-up-right" /></a>{filters.market === 'all' && <Link href={`${href('insights', item.city)}#local-conversation`}>{ko ? '이 도시의 이슈 5개 보기' : 'All five issues in this city'} <UiIcon name="arrow-right" /></Link>}</div></details>)}</div></section>
      {updates.length > 0 && <section className={styles.section} id="market-updates"><div className={styles.sectionHeading}><h2>{ko ? '시장을 읽는 데이터' : 'Market updates'}</h2><Link href={href('market')}>{ko ? '시장 분석 전체' : 'All market analysis'} <UiIcon name="arrow-right" /></Link></div><ArticleRows items={updates} /></section>}
    </>}
