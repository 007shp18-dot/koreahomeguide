...
export function NewsroomIndex({ articles, policies, filters, headlines, locale = 'en' }: Readonly<{
  ...
  const promoted = new Set(fullJourney ? [...notebookItems, ...comparisons, ...updates, ...pilotItems].map(item => item.href).concat(journeyArticleHref(story.city, 'discover', locale)) : []);
