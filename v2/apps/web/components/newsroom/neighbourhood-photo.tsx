...
export function NeighbourhoodPhoto({ id, eager = false, context = false }: Readonly<{ id: NeighbourhoodPhotoId; eager?: boolean; context?: boolean }>) {
  ...
  return <figure className={styles.scenePhoto} data-neighbourhood-photo={id}>
    ...
    <figcaption>{photo.caption}{context ? ' · City context' : ''}<details className={styles.photoCredit}><summary>Photo credit</summary><span><a href={photo.source} target="_blank" rel="noopener noreferrer">{photo.author}</a> · <a href={photo.licenseHref} target="_blank" rel="noopener noreferrer">{photo.license}</a></span></details></figcaption>
  </figure>;
}
