...
export function NeighbourhoodPhoto({ id, eager = false, context = false, locale = 'en' }: Readonly<{ id: NeighbourhoodPhotoId; eager?: boolean; context?: boolean; locale?: 'en' | 'ko' | 'zh-CN' }>) {
  ...
  return <figure className={`${styles.scenePhoto} ${photo.height > photo.width ? styles.portraitScene : ''}`} data-neighbourhood-photo={id}>
    <Image src={photo.src} alt={'alt' in photo ? photo.alt : photo.caption} width={photo.width} height={photo.height} loading={eager ? "eager" : "lazy"} fetchPriority={eager ? "high" : undefined} sizes="(max-width: 740px) calc(100vw - 40px), 900px" />
