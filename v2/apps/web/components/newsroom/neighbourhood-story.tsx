  ...
  return <main className={styles.page} lang="en">
    ...
    <article data-editorial-content-id={`en:${story.slug}`} data-editorial-content-type="guide" data-editorial-locale="en" data-editorial-market={markets[story.city as keyof typeof markets]}>
      ...
      <div className={styles.body}><p className={styles.intro}>{story.intro}</p><p className={styles.walk}><strong>Your loose plan</strong><br />{story.route}</p>
        ...
        <nav aria-label="Continue in this city" data-editorial-event="article_complete"><h2>Explore homes in {story.cityName}</h2><p><Link href={explores[story.city as keyof typeof explores] ?? '/prices/'} data-editorial-event="article_to_explore">Explore recorded transactions</Link> · <Link href={journeyArticleHref(story.city as keyof typeof markets, 'where', 'en')} data-editorial-event="article_open">Compare neighbourhoods</Link></p></nav>
