...
export function EditorialPortfolioIndex({ locale, records, section }: Readonly<{
  ...
  const copy = section === 'news'
    ? chinese
      ...
      : { eyebrow: 'SignedPrice Newsroom', title: 'Property change, checked against evidence.', deck: 'Reviewed policy, market and data reporting.' }
    : chinese
      ? { eyebrow: 'SignedPrice 指南', title: '先理解本地流程，再作跨境决定。', deck: '围绕租房、买房与成交数据的已核验实用指南。' }
      : { eyebrow: 'SignedPrice Guides', title: 'Understand the process before deciding.', deck: 'Reviewed guides for renting, buying and reading transaction evidence.' };
  return <main className={styles.index}>
    ...
    <section className={styles.latest} aria-labelledby="portfolio-list-title">
      <div className={styles.sectionHeading}><p>{records.length} reviewed records</p><h2 id="portfolio-list-title">{chinese ? '按读者问题开始' : 'Start with the reader question'}</h2></div>
