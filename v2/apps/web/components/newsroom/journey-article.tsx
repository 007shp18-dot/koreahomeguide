...
export function JourneyArticle({ article, locale }: Readonly<{ article: Article; locale: StoryLocale }>) {
  ...
  return <main className={styles.article} lang={locale} data-journey-article={`${article.city}/${article.id}`}>
    ...
    <article className={styles.body}>
      ...
      <section className={styles.nextAction} aria-label={ko ? '다음 단계' : 'Next step'}><p className={styles.eyebrow}>{ko ? '다음으로 해볼 일' : 'Your next step'}</p><h2><Link href={photoEssay ? exploreHref : actions.primary.href} data-editorial-event="journey_next">{photoEssay ? `Explore ${city.name.en}` : actions.primary.label[locale]} <UiIcon name="arrow-right" /></Link></h2></section>
