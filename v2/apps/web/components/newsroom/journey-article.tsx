...
export function JourneyArticle({ article, locale }: Readonly<{ article: Article; locale: StoryLocale }>) {
  const ko = locale === 'ko';
  const photoEssay = ko ? undefined : PHOTO_ESSAYS[`${article.city}/${article.id}`];
  ...
  const nextHref = photoEssay ? exploreHref : actions.primary.href;
  ...
  return <main className={styles.article} lang={locale} data-journey-article={`${article.city}/${article.id}`} data-editorial-content-id={`${locale}:${article.city}-${article.id}`} data-editorial-content-type="guide" data-editorial-locale={locale} data-editorial-market={{seoul:'kr-seoul',singapore:'sg-singapore',dubai:'ae-dubai',tokyo:'jp-tokyo'}[article.city]}>
    ...
    {!photoEssay && <nav className={styles.stages} aria-label={ko ? '도시 구매 여정' : 'City buying journey'}>{STORY_STEPS.map((item, index) => <Link key={item.id} href={journeyArticleHref(article.city, item.id, locale)} aria-current={item.id === article.id ? 'page' : undefined}><span>{String(index + 1).padStart(2, '0')}</span>{item.label[locale]}</Link>)}</nav>}
    {!photoEssay && <div className={styles.contents}><ArticleContents locale={locale} items={article.sections.map(section => ({ id: section.id, title: section.title[locale] }))} /></div>}
    <div className={styles.hero}>{photoEssay ? <NeighbourhoodPhoto id={photoEssay.hero} eager /> : article.id === 'wangsimni' || article.id === 'mangwon'
    ...
    <article className={styles.body}>
      {article.sections.map((section, sectionIndex) => <section key={section.id} id={section.id}>
        ...
        {!photoEssay && section.sourceIds.length > 0 && <p className={styles.references}>{ko ? '근거 자료' : 'Sources'} {section.sourceIds.map(id => <a key={id} href={`#source-${id}`} aria-label={`${ko ? '출처' : 'Source'} ${article.sources.findIndex(source => source.id === id) + 1}`}>[{article.sources.findIndex(source => source.id === id) + 1}]</a>)}</p>}
      </section>)}
      ...
      <section className={styles.nextAction} data-editorial-event="article_complete" aria-label={ko ? '다음 단계' : 'Next step'}><p className={styles.eyebrow}>{ko ? '다음으로 해볼 일' : 'Your next step'}</p><h2><Link href={nextHref} data-editorial-event={nextEvent}>{photoEssay ? `Explore ${city.name.en}` : actions.primary.label[locale]} <UiIcon name="arrow-right" /></Link></h2></section>
