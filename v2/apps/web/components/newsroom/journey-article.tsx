...
export function JourneyArticle({ article, locale }: Readonly<{ article: Article; locale: StoryLocale }>) {
  ...
  return <main className={styles.article} lang={locale} data-journey-article={`${article.city}/${article.id}`}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd({ '@context': 'https://schema.org', '@type': 'Article', headline: article.title[locale], description: article.deck[locale], inLanguage: locale, datePublished: article.checkedAt, dateModified: article.checkedAt, mainEntityOfPage: publicCanonical(href), author: { '@type': 'Organization', name: 'SignedPrice' }, publisher: { '@type': 'Organization', name: 'SignedPrice' }, citation: article.sources.map(source => source.href), isAccessibleForFree: true }) }} />
    ...
    <header className={styles.header}>
      ...
      <div className={styles.meta}><span>SignedPrice</span><span>{ko ? '자료 확인' : 'Sources checked'} <time dateTime={article.checkedAt}>{article.checkedAt.replaceAll('-', '.')}</time></span><span>{ko ? `${minutes}분 읽기` : `${minutes} min read`}</span><a href="#article-sources">{ko ? `출처 ${article.sources.length}개` : `${article.sources.length} sources`}</a></div>
    </header>
