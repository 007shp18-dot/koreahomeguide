...
export function JourneyArticle({ article, locale }: Readonly<{ article: Article; locale: StoryLocale }>) {
  ...
  return <main className={styles.article} lang={locale} data-journey-article={`${article.city}/${article.id}`} data-interface-page="article"
      data-editorial-content-id={`${locale}:${article.city}-${article.id}`} data-editorial-content-type="guide" data-editorial-locale={locale} data-editorial-market={{seoul:'kr-seoul',singapore:'sg-singapore',dubai:'ae-dubai',tokyo:'jp-tokyo'}[article.city]}>
    ...
    <article className={styles.body}>
      ...
      <details className={styles.sources} id="article-sources"><summary>{ko ? '참고 자료 보기' : 'Sources and further reading'}</summary><ol>{article.sources.map(source => <li key={source.id} id={`source-${source.id}`}><a href={source.href} target="_blank" rel="noopener noreferrer">{source.title}</a></li>)}</ol></details>
    </article>
  </main>;
}
