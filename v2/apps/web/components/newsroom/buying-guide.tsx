...
export function BuyingGuide({ guide }: Readonly<{ guide: BuyingGuideData }>) {
  ...
  return <div className={styles.guide}>
    ...
    <section aria-labelledby="buying-examples">
      ...
      <div className={styles.budgets} role="group" aria-label="Purchase-price ceiling">{guide.bands.map((band, index) => <button type="button" key={band.cap} aria-pressed={index === selected} onClick={() => setSelected(index)}>{money(band.cap)}</button>)}</div>
      ...
      <div className={styles.examples}>{budget.examples.map(example => <div className={styles.example} key={`${selected}-${example.name}-${example.band}`}>
        ...
        <details><summary>View transaction evidence</summary><p className={styles.meta}>Same-project {example.band}–&lt;{example.band + 20} m² group. Only records in the selected price range are listed.</p><div className={styles.tableScroll}><table><thead><tr><th>Date</th><th>m²</th><th>Price</th></tr></thead><tbody>{example.records.map((record, index) => <tr key={index}><td>{record.date}</td><td>{record.area.toFixed(2)}</td><td>{money(record.price)}</td></tr>)}</tbody></table></div></details>
      </div>)}</div></div>
    </section>
    <section aria-labelledby="buying-costs"><h2 id="buying-costs">02 · Budget beyond the price</h2>
      {guide.currency === 'SGD' ? <label className={styles.profile}>Buyer profile<select value={profile} onChange={event => setProfile(Number(event.target.value))}><option value={0}>Foreign individual · no remission</option><option value={1}>Singapore PR · first home</option><option value={2}>Singapore citizen · first home</option></select></label> : null}
    ...
    <section aria-labelledby="buying-checklist"><h2 id="buying-checklist">04 · Before you commit</h2>{guide.checks.map(check => <label className={styles.check} key={check}><input type="checkbox" />{check}</label>)}<p className={styles.meta}>Checklist selections are not saved.</p></section>
