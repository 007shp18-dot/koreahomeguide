...
export function BuyingGuide({ guide, locale = 'en' }: Readonly<{ guide: BuyingGuideData; locale?: 'en' | 'ko' }>) {
  ...
  return <div className={styles.guide}>
    ...
    <section aria-labelledby="buying-examples">
      ...
      <p className={styles.meta}>{t("Three projects with repeated records. Areas are observed ranges, not the maximum or average size available at this budget. Seoul project names follow the original Korean records.", "비슷한 가격대에서 거래가 반복된 단지 3곳입니다. 면적은 해당 거래의 범위로, 이 예산에서 살 수 있는 최대 면적이나 평균 면적은 아닙니다.")}</p>
