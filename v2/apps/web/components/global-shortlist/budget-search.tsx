...
import { shortlistText, shortlistHref, type ShortlistLocale } from './copy';

const empty = () => '';
export function BudgetSearch({ locale = 'en', market, embedded = false }: { market: OverseasMarket; embedded?: boolean } & { locale?: ShortlistLocale }) {
  const t = <T,>(value: T): T => shortlistText(locale, value);

  const zh = locale === 'zh-CN';
  const sg = market === 'singapore', jp = market === 'tokyo', currency = sg ? 'SGD' : jp ? 'JPY' : 'AED';
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const next = { ...filters, budget: Number(form.get('budget')), region: String(form.get('region')), housing: String(form.get('housing')), ...((sg || jp) ? { minArea: Number(form.get('minArea')), maxArea: Number(form.get('maxArea')) } : { completion: String(form.get('completion')) }) };
  ...
  function card(item: BudgetItem, savedCard = false) {
    ...
    return <article className={styles.card} key={item.key}><p className={styles.meta}>{t(sg ? item.region : jp ? 'Neighbourhood-level evidence' : 'Area-level evidence')}</p><h3>{t(item.name)}</h3>
      {savedCard && changed && <p className={styles.badge}>{t("Evidence updated")}</p>}
      <p className={styles.meta}>{t(sg ? savedCard ? 'Latest recorded resale · all published sizes and types' : 'Latest matching resale' : jp ? 'Median condominium price · selected size range' : 'Median transaction price · all sizes')}</p><p className={styles.price}>{t(currency)} {t(item.price.toLocaleString('en-US'))}</p><p>{t(item.description)}</p>
      ...
      <div className={styles.actions}><Link href={shortlistHref(locale, item.evidenceHref)}>{t("View evidence")}</Link><Link href={shortlistHref(locale, item.checkHref)}>{t(jp ? 'Calculate a JPY scenario' : 'Check an asking price')}</Link><button className={styles.saveAction} type="button" aria-pressed={Boolean(baseline)} onClick={() => baseline ? remove(item.key) : save(item)}><span aria-hidden="true">{baseline ? '♥' : '♡'}</span> {t(baseline ? 'Saved' : sg ? 'Save project' : 'Save area')}</button>
      ...
      {savedCard && <ResearchNote market={market} placeKey={item.key} placeName={item.name} locale={locale} />}
    </article>;
  }
  return <Root className={styles.page}><header className={styles.heading}><Link href={shortlistHref(locale, sg ? '/sg/singapore/explore/' : jp ? '/jp/tokyo/explore/' : '/ae/dubai/explore/')}><UiIcon name="arrow-left" /> {locale === 'ko' ? `${sg ? '싱가포르' : jp ? '도쿄' : '두바이'} 탐색` : zh ? `${sg ? '新加坡' : jp ? '东京' : '迪拜'} 探索` : `${sg ? 'Singapore' : jp ? 'Tokyo' : 'Dubai'} Explore`}</Link><p className={styles.eyebrow}>{t(sg ? 'SINGAPORE · PRIVATE RESALE' : jp ? 'TOKYO · CONDOMINIUM TRANSACTIONS' : 'DUBAI · AREA PRICE SCREEN')}</p><h1>{t(sg ? 'Find projects within your budget' : 'Find areas by median sale price')}</h1><p>{t(sg ? 'Search recorded resales, save projects and check an asking price against local evidence.' : jp ? 'Compare neighbourhood condominium medians, save areas and calculate your own JPY scenario.' : 'Screen area medians against your budget, save areas and continue to a price check.')}</p></header>
    <ShortlistCities locale={locale} current={market} /><form className={styles.form} key={filterKey} onSubmit={submit} aria-label={t("Budget search conditions")}>
      ...
      <label>{t(sg ? 'Market segment' : jp ? 'Ward' : 'Area')}<select name="region" defaultValue={filters.region}><option value="all">{t(sg ? 'All released segments' : 'All released areas')}</option>{(data?.regions ?? (sg ? ['CCR','RCR','OCR'].map(value => ({ value, label: value })) : [{ value: filters.region, label: filters.region }].filter(r => r.value !== 'all'))).map(r => <option key={r.value} value={r.value}>{t(r.label)}</option>)}</select></label>
      <label>{t("Property type")}<select name="housing" defaultValue={filters.housing}>{(sg ? ['condominium', 'apartment', 'executive_condominium'] : jp ? ['apartment'] : ['apartment', 'villa']).map(type => <option value={type} key={type}>{t(type.replaceAll('_', ' '))}</option>)}</select></label>
      {(sg || jp) ? <><label>{t(jp ? "Minimum reported area · m²" : "Minimum strata area · m²")}<DefaultAmountInput name="minArea"  min="1" max="2000" step="0.01" required defaultValue={filters.minArea} /></label><label>{t(jp ? "Maximum reported area · m²" : "Maximum strata area · m²")}<DefaultAmountInput name="maxArea"  min="1" max="2000" step="0.01" required defaultValue={filters.maxArea} /></label></> : <label>{t("Completion")}<select name="completion" defaultValue={filters.completion}><option value="ready">{t("Ready")}</option><option value="off-plan">{t("Off-plan")}</option></select></label>}
      <button className={styles.primary} type="submit">{t("Find & save conditions")}</button></form>
    <p className={styles.notice}>{t(sg ? 'Search covers the latest three months of released single-unit private resales; HDB is not included. These are recorded sales, not available listings. Strata area is not the same as Seoul net area.' : jp ? 'This screens anonymous government condominium transactions by neighbourhood and size. Each ward uses its latest published quarter, shown on every result. Saved evidence follows the selected size range. These are not named buildings or available listings.' : 'This filters area medians, not individual homes. A median within your budget does not establish that a particular home is available. Unit size and bedroom counts are not available in this release.')}{t(" Purchase price only; taxes, fees, financing and buyer eligibility are excluded. Coverage is limited to the released dataset.")}</p>
