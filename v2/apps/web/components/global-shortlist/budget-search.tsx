...
const empty = () => '';
export function BudgetSearch({ locale = 'en',  market }: { market: OverseasMarket } & { locale?: MarketLocale }) {
  ...
  function card(item: BudgetItem, savedCard = false) {
    ...
    return <article className={styles.card} key={item.key}><p className={styles.meta}>{t(sg ? item.region : 'Area-level evidence')}</p><h3>{t(item.name)}</h3>
      ...
      <div className={styles.actions}><Link href={marketHref(locale, item.evidenceHref)}>{t("View evidence")}</Link><Link href={marketHref(locale, item.checkHref)}>{t("Check an asking price")}</Link><button type="button" onClick={() => baseline ? remove(item.key) : save(item)}>{t(baseline ? 'Remove saved' : sg ? 'Save project' : 'Save area')}</button>
  ...
  return <main className={styles.page}><header className={styles.heading}><p className={styles.eyebrow}>{t(sg ? 'SINGAPORE · PRIVATE RESALE' : 'DUBAI · AREA PRICE SCREEN')}</p><h1>{t(sg ? 'Find projects within your budget' : 'Find areas by median sale price')}</h1><p>{t(sg ? 'Search recorded resales, save projects and check an asking price against local evidence.' : 'Screen area medians against your budget, save areas and continue to a price check.')}</p></header>
    <ShortlistCities locale={locale} current={market} /><form className={styles.form} key={filterKey} onSubmit={submit} aria-label={t("Budget search conditions")}>
      <label>{t("Purchase-price ceiling · ")}{t(currency)}<input name="budget" type="number" min="1" max="500000000" required defaultValue={filters.budget} /></label>
      ...
      {sg ? <><label>{t("Minimum strata area · m²")}<input name="minArea" type="number" min="1" max="2000" step="0.01" required defaultValue={filters.minArea} /></label><label>{t("Maximum strata area · m²")}<input name="maxArea" type="number" min="1" max="2000" step="0.01" required defaultValue={filters.maxArea} /></label></> : <label>{t("Completion")}<select name="completion" defaultValue={filters.completion}><option value="ready">{t("Ready")}</option><option value="off-plan">{t("Off-plan")}</option></select></label>}
