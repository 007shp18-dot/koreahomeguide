...
export function ContractRankings({ rows, checkedAt, city, kind, order = 'highest' }: {
 ...
 return <section className={styles.section} aria-labelledby="contract-ranking-title">
  <nav className={styles.tabs} aria-label="Transaction rankings">
   {(['seoul', 'singapore'] as const).flatMap(market => (['sale', 'rent'] as const).map(transaction =>
    <Link key={`${market}-${transaction}`} href={`/rankings/?city=${market}&kind=${transaction}&order=${order}`} aria-current={market === city && transaction === kind ? 'page' : undefined}>
     {market === 'seoul' ? 'Seoul' : 'Singapore'} {transaction === 'sale' ? 'sales' : 'rents'}
    </Link>))}
  </nav>
