    ...
    <main className={styles.main}>
      <ResearchPageHeading
        ...
        description={ko
          ...
          : 'Compare reported prices across Seoul buildings and Singapore projects, ranked from highest to lowest for each source period.'}
      ...
      <section className={styles.section} aria-labelledby="ranking-markets-title">
        ...
        <div className={styles.productGrid}>
          <Link href={`${prefix}/kr/seoul/rankings/`}><span>{ko ? '서울 · 건물' : 'Seoul · buildings'}</span><h3>{ko ? '건물별 매매·전세·월세 중앙값' : 'Sale, jeonse and rent by building'}</h3><p>{ko ? '각 순위에 표시된 기간의 국토교통부 신고 자료로 건물별 가격을 비교하고 자치구별 집계도 확인합니다.' : 'Compare building prices from MOLIT filings, with district context and the source period shown.'}</p><strong>{ko ? '서울 순위 보기 →' : 'Open Seoul rankings →'}</strong></Link>
