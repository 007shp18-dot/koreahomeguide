...
import { TokyoBudgetChart } from '../visual/tokyo-budget-chart';
import { IllustratedCta } from '../visual/illustrated-cta';
import visualStyles from '../market-ui/market-overview.module.css';
...
export default async function TokyoOverview({ searchParams, locale = 'en' }: { searchParams: Promise<Params>; locale?:TokyoLocale }) {
  ...
  return <>
    ...
    <main className={styles.main} data-tokyo-overview>
      <PhotographicHero city="tokyo" locale={locale} title={text('Tokyo','도쿄','东京')} description={text('Explore neighbourhoods through recorded prices, home sizes and quarterly sales.','분기별 실거래가와 주택 면적으로 동네를 비교하세요.','通过季度成交价格与住房面积比较社区。')} media={<MarketRepresentativePhoto photo={MARKET_PHOTOS.tokyo} context="city" cityLabel={text('Tokyo','도쿄','东京')} locale={locale} eager />} />
      <div className={visualStyles.content}>
      <TokyoNavigation locale={locale} current="overview" />
      <TokyoBudgetChart locale={locale} />
      <LocalLifeGallery city="tokyo" locale={locale} />
