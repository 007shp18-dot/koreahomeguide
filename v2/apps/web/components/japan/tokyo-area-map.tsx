...
export function TokyoAreaMap({ rows, city, year, quarter, browserKey, filters, unavailable = false, locale = 'en' }: { locale?: TokyoLocale;
  ...
  const visibleWards = TOKYO_WARDS.filter(([, name]) => name.toLowerCase().includes(wardSearch.trim().toLowerCase()));
  ...
  return <section className={styles.areaMap} aria-label="Tokyo area price map" data-tokyo-google-map="true" aria-busy={pending}>
    ...
    <details className={styles.wardDirectory}><summary><span>{wardName}</span><span>{t('Change ward · 23 wards')}</span></summary>
      ...
      <nav className={styles.mapAreaList} aria-label="Choose a Tokyo ward">{visibleWards.map(([code, name]) => {
        ...
        return <Link key={code} data-ward={code} aria-current={code === city ? 'location' : undefined} prefetch={false} scroll={false}
          href={tokyoMapAreaHref(summary ?? { city: code, year, quarter, district: null }, filters, locale)}>{name}<span>{summary ? `${summary.year} Q${summary.quarter}` : t('View coverage')}</span></Link>;
      })}</nav>
    ...
    <section className={styles.neighbourhoods} aria-label={`Neighbourhoods in ${wardName}`}>
      ...
      {!neighbourhoods.length ? <p>{unavailable ? t('Neighbourhood summaries are temporarily unavailable. You can still search the transaction records.') : t('No neighbourhoods match this period and property filters. Try another quarter or a wider area range.')}</p> : <>
        <label className={styles.areaSearch}>{t('Find a neighbourhood')}<input type="search" value={neighbourhoodSearch} onChange={event => setSearch({ city, value: event.target.value })} placeholder={`Search ${wardName} neighbourhoods`} /></label>
