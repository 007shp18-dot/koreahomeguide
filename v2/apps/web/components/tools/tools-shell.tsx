...
export function ToolsShell({locale='en',href,children}:Readonly<{locale?:SiteLocale;href:string;children:ReactNode}>) {
 ...
 return <><SiteHeader copy={{brand:'signedprice',homeLabel:'signedprice home',navigationLabel:'Tools navigation',languageLabel:locale==='ko'?'KO':locale==='zh-CN'?'ZH':'EN',links:[{label:name,href,isCurrent:true}]}}/><main className={styles.page}>{children}</main><SiteFooter copy={{brand:'signedprice',descriptor:'Property research and practical decision tools.',navigationLabel:'Footer',links:[],status:'Source and assumptions are shown with each result.'}}/></>;
}
