...
export function ToolsShell({locale='en',href,children}:Readonly<{locale?:SiteLocale;href:string;children:ReactNode}>) {
 ...
 return <><SiteHeader copy={{homeHref:locale==='ko'?'/ko/':locale==='zh-CN'?'/zh-cn/':'/',brand:'signedprice',homeLabel:'signedprice home',navigationLabel:'Tools navigation',languageLabel:locale==='ko'?'KO':locale==='zh-CN'?'ZH':'EN',links:[{label:name,href,isCurrent:true}]}}/><main data-interface-page="tools" className={styles.page}>{children}</main><SiteFooter locale={locale} copy={{brand:'signedprice',descriptor:'Property research and practical decision tools.',navigationLabel:'Footer',links:[],status:'Source and assumptions are shown with each result.'}}/></>;
}
