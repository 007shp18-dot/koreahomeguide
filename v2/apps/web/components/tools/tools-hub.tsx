...
export function ToolsHub({ locale = 'en' }: Readonly<{ locale?: SiteLocale }>) {
  ...
  const items: readonly ToolDirectoryItem[] = [
    ...
    { group: 'check', title: ko ? '서울 임대료 확인' : zh ? '核对首尔租金' : 'Check a Seoul rent quote', description: ko ? '보증금과 월세를 신고된 임대차 계약과 비교하세요.' : zh ? '使用公开租赁资料核对输入的条件。' : 'Compare your rent and deposit with reported rental contracts.', market: 'kr-seoul', tool: 'rent-check', href: `${prefix}/kr/seoul/check/?transaction=monthly` },
    ...
    { group: 'check', title: ko ? '도쿄 매물 가격 비교' : zh ? '核对东京报价' : 'Check a Tokyo asking price', description: ko ? '매물의 면적·가격을 입력해 같은 구·유사 면적의 정부 공동주택 거래와 비교하세요.' : zh ? '输入面积和报价，与同区相近面积的政府公寓成交资料比较。' : 'Compare your asking price per m² with official condominium transactions for the same ward and size range.', market: 'jp-tokyo', tool: 'tokyo-budget', href: zh ? '/zh-cn/jp/tokyo/tools/' : '/jp/tokyo/tools/' },
  ];
    ...
    <div className={styles.directory}>
      {groups.map((group) => <section id={`tool-${group.id}`} className={styles.group} data-tool-group={group.id} key={group.id}>
        ...
        <ul className={styles.list}>{items.filter((item) => item.group === group.id).map((item) => <li key={item.tool} data-tool-id={item.tool} data-tool-market={item.market}>
          ...
          <TrackedToolLink href={marketHref(locale, item.href)} market={item.market} tool={item.tool} surface="tools-hub">{ko ? '열기' : zh ? '打开' : 'Open tool'}<UiIcon name="arrow-right" /></TrackedToolLink>
        </li>)}</ul>
      </section>)}
    </div>
    <p><Link href="/privacy/#research-contributions">{ko ? '개인정보 페이지에서 연구 기여 자료 관리' : zh ? '在隐私页面管理研究提交内容' : 'Manage research contributions on the Privacy page'}</Link></p>
    <section className={`${styles.group} ${styles.budget}`}><div className={styles.groupHeading}><h2>{ko ? '예산으로 단지 찾기' : zh ? '按预算寻找住宅' : 'Find homes within your budget'}</h2><p>{ko ? '예산 안에서 거래된 단지를 찾아 이 브라우저에 저장할 수 있습니다. 현재 판매 중인 매물은 별도로 확인해 주세요.' : zh ? '根据已申报交易筛选住宅，并在当前浏览器保存关注列表。不是在售房源。' : 'Find places with reported prices within your budget and save a shortlist in this browser. These are transaction screens, not available listings.'}</p></div><nav className={styles.links} aria-label={ko ? '지역별 예산 검색' : 'Budget search by city'}><Link href={`${prefix}/kr/seoul/shortlist/`}>{ko ? '서울' : 'Seoul'}</Link><Link href={marketHref(locale, '/sg/singapore/shortlist/')}>{ko ? '싱가포르' : 'Singapore'}</Link><Link href={marketHref(locale, '/ae/dubai/shortlist/')}>{ko ? '두바이' : 'Dubai'}</Link><Link href={marketHref(locale, '/jp/tokyo/shortlist/')}>{ko ? '도쿄' : 'Tokyo'}</Link></nav></section>
  </ToolsShell>;
}
