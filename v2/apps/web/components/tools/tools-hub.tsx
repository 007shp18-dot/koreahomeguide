...
export function ToolsHub({ locale = 'en' }: Readonly<{ locale?: SiteLocale }>) {
  ...
  const items: readonly ToolDirectoryItem[] = [
    { group: 'compare', title: ko ? '같은 예산으로 세 도시 비교' : zh ? '比较三座城市的预算购买力' : 'Compare one budget across three cities', description: ko ? 'USD·KRW·SGD·AED 예산으로 서울·싱가포르·두바이를 비교합니다.' : zh ? '选择美元、韩元、新加坡元或迪拉姆预算，比较三座城市。' : 'See what a USD, KRW, SGD or AED budget could buy in Seoul, Singapore and Dubai, based on recorded prices.', market: 'global', tool: 'passport', href: ko ? '/ko/passport/' : zh ? '/zh-cn/passport/' : '/passport/' },
    ...
    { group: 'returns', title: ko ? '매입 비용·임대수익 계산' : zh ? '计算购置成本与运营收益率' : 'Calculate purchase costs and operating yield', description: ko ? '매입 비용, 예상 임대료와 공실 기간을 입력해 계산하세요.' : zh ? '自行输入 KRW、SGD 或 AED 成本、租金及空置假设。' : 'Enter your own costs, rent and vacancy in KRW, SGD or AED.', market: 'global', tool: 'property-scenario', href: `${prefix}/tools/property-scenario/` },
