...
import { COLLECTION_CITIES, matchesMarket, matchesSource, marketJobLabel, sourceSignals, type CollectionFilter } from '../../lib/data-operations/collection-view';
import styles from './workspace.module.css';

type Publication = {sourceAsOf:string;releasedAt:string;saleCount:number;rentCount:number;digest:string};
type Dashboard = {recovery?: {source_host:string;state:string;unique_images:number;candidate_rows:number;multiple_building_images:number}[];media?: MediaPipelineRow[];quality?: DataQuality[]; markets?: MarketCollectionStatus[]; sources: CollectionStatus[]; publication: string};
...
export function CollectionPanel({initialData, onUnauthorized}: {initialData?: Dashboard; onUnauthorized?: () => void}) {
 ...
 const [city,setCity]=useState('all');const [filter,setFilter]=useState<CollectionFilter>('all');const [query,setQuery]=useState('');
 ...
 const load = useCallback(() => request<Dashboard>(endpoint).then(result=>{setData(result);setError('');setRefreshedAt(Date.now());}).catch(cause=>setError(cause instanceof Error ? cause.message : '조회 실패')).finally(()=>setLoading(false)),[request]);
 const loadPublication = useCallback(() => {setPublicationLoading(true);return request<{publication:Publication|null}>('/api/internal/singapore-publication/').then(result=>{setPublication(result.publication);setPublicationError('');}).catch(()=>setPublicationError('싱가포르 공개 상태를 확인하지 못했습니다.')).finally(()=>setPublicationLoading(false));},[request]);
 ...
 const visibleSources=(data?.sources??[]).filter(s=>(city==='all'||s.market===city)&&matchesSource(s,filter,refreshedAt)&&`${s.name} ${s.sourceId} ${COLLECTION_CITIES.find(city=>city.id===s.market)?.name??''} ${s.lastError??''}`.toLocaleLowerCase().includes(needle));
 const visibleMarkets=(data?.markets??[]).filter(m=>(city==='all'||m.job.startsWith(COLLECTION_CITIES.find(c=>c.id===city)?.prefix??'__unknown__'))&&matchesMarket(m,filter)&&`${m.job} ${marketJobLabel(m.job)} ${m.errorCode??''}`.toLocaleLowerCase().includes(needle));
 return <section aria-label="정기 수집 운영">
  <div className={styles.detailHeader}><div><h2>수집 운영</h2><p>어디서 멈췄는지 확인하고, 필요한 작업만 실행합니다.</p></div><button type="button" disabled={loading||publicationLoading||busy} onClick={()=>{setLoading(true);void load();void loadPublication();}}>{loading||publicationLoading?'조회 중…':'수집·공개 상태 새로고침'}</button></div>
  {refreshedAt&&<p className={styles.hint}>화면 갱신 {formatTime(new Date(refreshedAt).toISOString())} · 아래 상태는 이 시각 기준입니다.</p>}
  {data&&<CollectionOverview sources={data.sources} markets={data.markets} now={refreshedAt} city={city} onCity={setCity} publicationAt={publication?.releasedAt??null} publicationLoading={publicationLoading} publicationError={publicationError}/>}
  <details className={styles.collectionTools}><summary>검증·공개·건물 보강 작업</summary><p>아래 실행은 수동 작업입니다. 현황 조회만으로 수집하거나 사이트에 공개하지 않습니다.</p>
  <section className={styles.notice} aria-label="싱가포르 화면 데이터 공개"><h3>싱가포르 거래 자료 · 화면 반영</h3>{publicationError?<p role="alert">{publicationError}</p>:publication?<p>수집 기준 {formatTime(publication.sourceAsOf)} · 마지막 공개 {formatTime(publication.releasedAt)} · 매매 {publication.saleCount.toLocaleString()}건 · 임대 {publication.rentCount.toLocaleString()}건</p>:<p>{publicationLoading?'공개 버전 조회 중…':'공개한 버전 기록이 없습니다.'}</p>}<button type="button" disabled={busy} onClick={()=>void publishSingapore()}>싱가포르 거래 검증 후 사이트 반영</button><p>수집 원문·세금·요금표의 검토 상태와 별도로, 검증을 통과한 싱가포르 거래 자료를 반영합니다.</p></section>
  ...
  {!data ? <p role="status">{loading?'출처별 수집 현황을 불러오고 있습니다…':'수집 현황에 연결하지 못했습니다.'}</p> : <>
   ...
   <p role="status" className={styles.hint}>거래 작업 {visibleMarkets.length}개 · 출처 {visibleSources.length}개 표시{filter==='overdue'?' · 예정 시각이 있는 출처만 포함하며 거래 작업의 지연을 추정하지 않습니다.':''}</p>
   {!visibleMarkets.length&&!visibleSources.length&&<p className={styles.empty}>선택한 조건에 맞는 항목이 없습니다.</p>}
   <details className={styles.collectionTools}><summary>자료 품질·사진 파이프라인 상세</summary>
   {data.quality && <DataQualityPanel rows={data.quality}/>}
   {data.media && <MediaPipelinePanel rows={data.media}/>}
   {data.recovery && <section aria-label="사진 원문 복구"><h3>사진 원문 복구 · 중복 URL 기준</h3><p>같은 이미지 주소는 한 번만 조사합니다. 출처 복구와 건물 일치 검수는 별개이며, 여러 건물에 연결된 사진은 별도로 확인해야 합니다.</p><div className={styles.tableScroll}><table><thead><tr><th>이미지 서버</th><th>원문 상태</th><th>고유 URL</th><th>연결 기록</th><th>여러 건물 연결 URL</th></tr></thead><tbody>{data.recovery.map(r=><tr key={`${r.source_host}:${r.state}`}><td>{r.source_host}</td><td>{{pending:'복구 대기',recovered:'원문 정보 복구',retry:'재시도 대기','source-unresolved':'원문·허가 출처 확보 필요'}[r.state]??r.state}</td><td>{r.unique_images.toLocaleString()}</td><td>{r.candidate_rows.toLocaleString()}</td><td>{r.multiple_building_images.toLocaleString()}</td></tr>)}</tbody></table></div></section>}
   </details>
   {visibleMarkets.length>0 && <div className={styles.tableScroll}><table><caption>공식 매매·임대 수집 · 건수는 마지막 시도 기준 · 성공 시각과 구분</caption><thead><tr><th scope="col">작업</th><th scope="col">시각</th><th scope="col">마지막 시도 건수</th><th scope="col">오류·이상 징후</th></tr></thead><tbody>{visibleMarkets.map(market=><tr key={market.job}><td><strong>{marketJobLabel(market.job)}</strong><small>{market.job}</small><small>{market.enabled?'예약 수집 활성':'예약 수집 비활성'} · {({succeeded:'수집 성공',failed:'수집 실패',running:'수집 중',queued:'실행 대기'} as Record<string,string>)[market.state??'']??market.state??'실행 기록 없음'}</small></td><td><small>시도 {formatTime(market.lastAttemptAt)}</small><small>성공 {formatTime(market.lastSuccessAt)}</small><small>성공 실행의 기준 시각 {formatTime(market.sourceAsOf)}</small></td><td>{market.lastAttemptAt?<><small>수신 {market.received} · 신규 {market.inserted} · 수정 {market.updated}</small><small>변경 없음 {market.unchanged} · 연결 필요 {market.unlinked}</small></>:<span>— 실행 기록 없음</span>}</td><td><small>연속 실패 {market.consecutiveFailures}회</small>{market.errorCode && <strong>{market.errorCode}</strong>}{market.anomaly && <p role="status">{market.anomaly}</p>}</td></tr>)}</tbody></table></div>}
   <div className={styles.tableScroll}><table><caption>출처별 수집 상태 · 시각은 UTC · 신규·변경은 마지막 성공 실행 기준</caption><thead><tr><th scope="col">출처</th><th scope="col">수집 시각</th><th scope="col">변경·검토</th><th scope="col">실패·이상 징후</th><th scope="col">마지막 공개</th><th scope="col">작업</th></tr></thead><tbody>{visibleSources.map(source=><tr key={source.sourceId}>
    <td><strong>{source.name}</strong><small>{sourceSignals(source,refreshedAt).failed?'다음 작업: 오류와 마지막 실행 결과 확인':sourceSignals(source,refreshedAt).overdue?'다음 작업: 예정 시각 경과 원인 확인':sourceSignals(source,refreshedAt).pending>0?'다음 작업: 원문·건물·위치 후보 검토':source.mode==='blocked'?'다음 작업: 사용 허가·연결 조건 확인':!source.lastAttemptAt?'다음 작업: 최초 실행 설정 확인':'최근 수집 결과 확인'}</small><small>{source.market} · {source.category} · {source.intervalDays}일 간격</small><small>{source.mode==='page-monitor'?'공식 페이지 변경 확인':source.mode==='address-api'?'공식 주소 API':source.mode}</small><small>{source.limitation}</small><a href={source.url} target="_blank" rel="noreferrer">공식 출처</a></td>
    ...
    <td>{source.consecutiveFailures>0?<strong role="status">연속 실패 {source.consecutiveFailures}회</strong>:<span>{source.lastAttemptAt?'연속 실패 없음':'실행 기록 없음 · 정상 여부 미확인'}</span>}{source.lastError && <small>{source.lastError}</small>}{source.anomaly && <strong role="status">이상 징후: {source.anomaly}</strong>}{source.probe && <details><summary>최근 API 점검</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify(source.probe,null,2)}</pre></details>}</td>
