...
export function CollectionPanel({initialData, onUnauthorized}: {initialData?: Dashboard; onUnauthorized?: () => void}) {
 ...
 const loadPublication = useCallback(() => request<{publication:Publication|null}>('/api/internal/singapore-publication/').then(result=>{setPublication(result.publication);setPublicationError('');}).catch(()=>setPublicationError('싱가포르 공개 상태를 확인하지 못했습니다.')).finally(()=>setPublicationLoading(false)),[request]);
 ...
 return <section aria-label="정기 수집 운영">
  <div className={styles.detailHeader}><div><h2>수집 운영</h2><p>어디서 멈췄는지 확인하고, 필요한 작업만 실행합니다.</p></div><button type="button" disabled={loading||publicationLoading||busy} onClick={()=>{setLoading(true);setPublicationLoading(true);void load();void loadPublication();}}>{loading||publicationLoading?'조회 중…':'수집·공개 상태 새로고침'}</button></div>
