...
export function EvidenceAdmin({ initialAuthenticated, initialData = null }: { initialAuthenticated: boolean; initialData?: PoolData | null }) {
  ...
  const [tab, setTab] = useState<'evidence' | 'sources' | 'create' | 'research'>('evidence');
  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      <header className={styles.header}><div><p className={styles.eyebrow}>EVIDENCE WORKSPACE</p><h1>{tab === 'research' ? 'Tool 공유 데이터' : tab === 'sources' ? '출처 관리' : tab === 'create' ? '자료 등록' : '자료 검토'}</h1><p>{tab === 'sources' ? '자료를 가져온 출처부터 확인하세요.' : tab === 'create' ? '원문 대신 확인 가능한 값과 출처를 남기세요.' : '검토 대기 자료를 확인하고, 분석에 쓸 근거를 정리하세요.'}</p></div><button type="button" disabled={loading || busy} onClick={() => { setLoading(true); void load(); }}>{loading ? '불러오는 중…' : '새로고침'}</button></header>
        ...
        <section className={styles.panel}>
          ...
          {tab === 'evidence' && <>
            <form className={styles.filters} onSubmit={(e) => { e.preventDefault(); const values = new FormData(e.currentTarget); setSelected(null); setFilters({ ...filters, market: String(values.get('market')), status: String(values.get('status')), quality: String(values.get('quality')), q: String(values.get('q')).trim(), page: 1 }); }}>
              ...
              <label>수집 점검<select name="quality" defaultValue={filters.quality}><option value="">전체</option><Options values={qualityLabels} /></select></label>
