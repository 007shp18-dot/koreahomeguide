    ...
    <main className={styles.main}>
      ...
      {data && <div className={selectedRow ? styles.split : ''}>
        <section className={styles.panel}>
          ...
          {tab === 'evidence' && <>
            <form className={styles.filters} onSubmit={(e) => { e.preventDefault(); const values = new FormData(e.currentTarget); setSelected(null); setFilters({ ...filters, market: String(values.get('market')), status: String(values.get('status')), q: String(values.get('q')).trim(), page: 1 }); }}>
              <label>시장<select name="market" defaultValue={filters.market}><option value="">전체 시장</option><Options values={markets} /></select></label>
              <label>상태<select name="status" key={filters.status} defaultValue={filters.status}><option value="">전체 상태</option><Options values={statuses} /><option value="expired">유효기간 만료</option></select></label>
              <label>지역·건물·출처<input name="q" defaultValue={filters.q} maxLength={120} placeholder="검색어 입력" /></label><button type="submit" disabled={loading}>조회</button>
            </form>
            {!data.evidence.length ? <div className={styles.empty}><h2>{data.total === 0 && !filters.q && !filters.market && !filters.status ? '등록된 자료가 없습니다' : '조건에 맞는 자료가 없습니다'}</h2><p>출처 등록 → 자료 입력 → 출처 승인 → 자료 검토 순서로 진행하세요.</p><button type="button" onClick={() => setTab('sources')}>출처 관리</button></div> : <div className={styles.tableScroll}><table><caption className={styles.caption}>조회 {data.total}건 · 금액 단위와 성격을 확인하세요</caption><thead><tr><th scope="col">자료</th><th scope="col">금액</th><th scope="col">검토 상태</th><th scope="col">활용 준비</th></tr></thead><tbody>{data.evidence.map((row) => {
              const ready = readiness(row, today);
              return <tr key={row.id} className={selected?.id === row.id ? styles.selectedRow : ''}><td><button type="button" className={styles.rowLink} onClick={() => setSelected({ entity: 'evidence', id: row.id })}>{row.building || row.area}</button><small>{markets[row.market]} · {row.area} · {metrics[row.metric]}</small><small>{row.sourceName} · {row.observedOn}</small></td><td><strong>{row.amount.toLocaleString('en-US')} {row.currency}</strong><small>{bases[row.basis]} · {units[row.unit]}</small></td><td><span className={styles.badge} data-status={row.status}>{statuses[row.status]}</span><small>{tiers[row.tier]}</small></td><td><span>{ready.ready ? '기초 요건 충족' : ready.reasons[0]}</span><small>{ready.ready ? '동일 조건 비교는 별도 확인' : `확인 필요 ${ready.reasons.length}항목`}</small></td></tr>;
            })}</tbody></table></div>}
            <div className={styles.pagination}><button type="button" disabled={loading || filters.page <= 1} onClick={() => { setSelected(null); setFilters({ ...filters, page: filters.page - 1 }); }}>이전</button><span>{filters.page} / {Math.max(1, Math.ceil(data.total / 25))}</span><button type="button" disabled={loading || filters.page * 25 >= data.total} onClick={() => { setSelected(null); setFilters({ ...filters, page: filters.page + 1 }); }}>다음</button></div>
          </>}
        </section>
        {selected && selectedRow && <aside className={styles.panel} aria-label="선택 자료 상세"><div className={styles.detailHeader}><h2>{selected.entity === 'source' ? (selectedRow as Source).name : (selectedRow as Evidence).building || (selectedRow as Evidence).area}</h2><button type="button" onClick={() => setSelected(null)}>닫기</button></div>
          ...
          {selected.entity === 'evidence' && <div className={styles.notice}><strong>활용 준비 점검</strong><ul>{readiness(selectedRow as Evidence, today).reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul>{readiness(selectedRow as Evidence, today).ready && <p>기초 요건을 충족했습니다. 비교 대상의 시점·면적·항목·단위·금액 성격 일치는 별도로 확인해야 합니다.</p>}</div>}
