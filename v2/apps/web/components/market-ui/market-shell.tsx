...
export function MarketDetailShell({ breadcrumb, identity, metric, evidence, rail, media }: Readonly<{
  ...
  return <article className={styles.detail} data-market-detail-shell="true">
    ...
    <header className={styles.identity} data-has-media={media ? "true" : "false"}><div>{identity}</div><div className={styles.heroMedia}>{media ?? metric}</div></header>
    <nav className={styles.detailTabs} aria-label="Detail sections"><a href="#detail-overview">Overview</a><a href="#detail-evidence">Transactions</a><a href="#detail-source">Source</a></nav>
    <section className={styles.detailSummary} id="detail-overview"><div>{metric}</div><p>Reported transaction history. See the source and reporting period below.</p></section>
