...
export function MonthlyTransactionResearch({ months: releasedMonths, locale = 'en' }: Readonly<{ locale?: 'en' | 'ko' | 'zh-CN'; months: readonly ResearchMonth[] }>) {
  ...
  return <section className={styles.section} aria-labelledby="project-history-heading" data-transaction-research="monthly">
    ...
    <p className={styles.chartHint}>{localizedMarketCopy(locale, "On narrow screens, scroll the chart horizontally. Open monthly figures below for exact values.", "좁은 화면에서는 그래프를 좌우로 스크롤하세요. 정확한 값은 아래 월별 수치에서 확인하세요.")}</p>
    <div className={styles.chartWrap} role="region" aria-label={localizedMarketCopy(locale, "Monthly transactions chart — scroll horizontally", "월별 거래 차트 — 가로로 스크롤")} tabIndex={0}>
    <svg className={styles.chart} viewBox={`0 0 720 ${baseline + 36}`} role="img" aria-label={hasPrices ? (localizedMarketCopy(locale, "Monthly sale-price points and transaction-volume bars. Exact values are in the table below.", "월별 매매가와 거래량 막대그래프입니다. 정확한 값은 아래 표에서 확인하세요.")) : (localizedMarketCopy(locale, "Monthly reported transaction counts. Exact values are in the table below.", "월별 신고 거래량입니다. 정확한 값은 아래 표에서 확인하세요."))}>
      ...
      {months.map((month, i) => <g key={month.month}>
        {month.median === null ? null : <circle cx={x(i)} cy={priceY(month.median)} r="3.5" fill="currentColor"><title>{`${month.month}: ${money(month.median, 'SGD')} · ${month.count}${localizedMarketCopy(locale, " sales", "건")}`}</title></circle>}
        <rect x={x(i) - 3} y={baseline - (month.count === 0 ? 0 : Math.max(2, month.count / maxCount * 50))} width={Math.min(10, 500 / Math.max(months.length, 1))} height={month.count === 0 ? 0 : Math.max(2, month.count / maxCount * 50)} fill="currentColor" opacity=".5"><title>{`${month.month}: ${locale === 'ko' ? `신고 거래 ${month.count}건` : locale === 'zh-CN' ? `${month.count} 笔申报交易` : `${month.count} reported sales`}`}</title></rect>
