...
export function PropertyScenarioCalculator({ price, currency, locale = 'en' }: Readonly<{
  ...
  return <section className={styles.section} aria-labelledby={`${id}-heading`} data-property-scenario={currency}>
    ...
    <div className={styles.form}>{fields.map(([key, label]) => <label key={key} htmlFor={`${id}-${key}`}>{label}{key === 'vacancyMonths' ? '' : ` (${currency})`}<input id={`${id}-${key}`} type="number" inputMode="decimal" min="0" max={key === 'vacancyMonths' ? 12 : undefined} step="any" value={inputs[key]} placeholder={key === 'price' ? undefined : (ko ? '직접 입력 · 해당 없으면 0' : 'Enter assumption; 0 if none')} onChange={(event) => setInputs((current) => ({ ...current, [key]: event.target.value }))} /></label>)}</div>
    {scenario ? <dl className={styles.results} aria-live="polite">
      ...
      <div><dt>{ko ? '취득 비용 대비 운영 수익률' : 'Operating yield on total cost'}</dt><dd>{scenario.netYield.toFixed(2)}%</dd></div>
    </dl> : <p role="status">{ko ? '모든 가정을 입력하면 계산 결과가 표시됩니다. 해당 없는 비용은 0을 입력하세요.' : 'Complete every assumption to calculate. Enter 0 for costs that do not apply.'}</p>}
    <p>{ko ? '계산식: (월 임대료 × (12 − 공실 개월) − 연 운영 비용) ÷ (매입 가격 + 취득 비용). 차입금·이자·양도차익·개인 소득세·환율 변화는 반영하지 않습니다. 실제 수익률 예측이 아닙니다.' : 'Formula: (monthly rent × (12 − vacant months) − annual operating costs) ÷ (price + acquisition costs). Excludes financing, capital gains, personal income tax and exchange-rate changes. Results reflect only the assumptions entered.'}</p>
  </section>;
}
