...
function ReportedContractMiniChart({ model, locale }: Readonly<{ model: KoreaExplorerBuildingDetailModel; locale: Locale }>) {
  ...
  return <figure className={styles.miniChart} data-building-summary-chart="reported-contracts">
    <figcaption>{locale === 'ko' ? '신고 거래' : locale === 'zh-CN' ? '申报交易' : 'Reported sales'} · {rows[0]!.areaSqm}㎡</figcaption>
    <svg viewBox="0 0 200 96" role="img" aria-label={locale === 'ko' ? '같은 전용면적의 실제 신고 거래. 점은 계약이며 가격 지수가 아닙니다.' : locale === 'zh-CN' ? '相同专有面积的实际申报合同。每个点代表一笔合同，并非价格指数。' : 'Actual reported contracts at the same unit size. Dots are contracts, not a price index.'}>
    ...
    <a href="#building-transactions">{locale === 'ko' ? `${rows.length}건 거래표 보기` : locale === 'zh-CN' ? `查看${rows.length}笔合同` : `View ${rows.length} contracts`}</a>
  </figure>;
}
