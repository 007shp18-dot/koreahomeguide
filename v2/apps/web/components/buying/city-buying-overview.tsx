...
import { CITY_BUYING, cityText, cityPrefix, consultationHref } from '../../content/city-buying-content';
...
export function CityBuyingOverview({city,locale}:{city:BuyingCity;locale:ContentLocale}) {
 ...
 return <section className={styles.section} data-city-buying={city} aria-labelledby="city-buying-title">
  ...
  <div className={styles.columns}>
   <article><h3>{t(['The tradeoff to check','함께 봐야 할 제약','同时核查的限制'])}</h3><p>{t(model.tradeoff)}</p><a href={model.source}>{t(['Official source','공식 출처','官方来源'])}</a><small>{t(['Editorial lens · sources checked 15 Sep 2026. No return forecast.','편집 관점 · 출처 확인 2026.09.15 · 수익을 예측하지 않습니다.','编辑视角 · 来源核查2026.09.15 · 非回报预测。'])}</small></article>
   <article><h3>{t(['Start with your budget','예산부터 좁혀보기','从预算开始'])}</h3><p>{guide.budgets[locale]}</p><p className={styles.muted}>{city==='tokyo'?'2026 Q1':guide.period[locale]} · {t(['Historical transactions','과거 거래 기준','历史成交'])}</p><Link href={`${guidePrefix}/guides/${guide.slug}/`}>{t(['Budget examples','예산별 거래 사례','预算成交案例'])}{guidePrefix!==prefix?' · English':''}</Link></article>
