...
export async function SingaporeRentalEvidence({ projectId, saleDigest, locale = 'en' }: { projectId: string; saleDigest: string; locale?: 'en' | 'ko' | 'zh-CN' }) {
  ...
  return <section className="mx-auto max-w-7xl px-4 py-6" aria-label={localizedMarketCopy(locale, "Official rental evidence", "공식 임대 계약 근거")}>
    ...
    {rows.length === 0 ? <p className="mt-3 text-sm">{localizedMarketCopy(locale, "Insufficient contracts with comparable conditions for publication.", "같은 조건으로 공개할 수 있는 계약 표본이 부족합니다.")}</p> : <div className="mt-4 overflow-x-auto"><table className="min-w-[720px] w-full text-left text-sm"><thead><tr>{(ko ? ['계약월','유형','침실','면적 범위 (㎡)','계약 수','월 임대료 (SGD)'] : locale === 'zh-CN' ? ['月份','类型','卧室','面积范围 (㎡)','合同数','月租 (SGD)'] : ['Month','Type','Bedrooms','Area band (㎡)','Contracts','Monthly rent (SGD)']).map(label => <th className="whitespace-nowrap p-2" key={label}>{label}</th>)}</tr></thead><tbody>{rows.slice(0,24).map((row,index) => <tr key={index} className="border-t"><td className="whitespace-nowrap p-2">{row.month}</td><td className="p-2">{row.propertyType}</td><td className="p-2">{row.bedrooms ?? '—'}</td><td className="p-2">{row.areaRange}</td><td className="p-2">{row.n}</td><td className="p-2">{row.medianMonthlySgd.toLocaleString('en-SG')}</td></tr>)}</tbody></table></div>}
