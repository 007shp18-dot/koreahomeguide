...
function MarketTabs({ locale = 'en', prefix, model }: Readonly<{ locale?: MarketLocale; prefix: 'a' | 'b'; model: SingaporeCheckRouteModel }>) {
  ...
  return <nav className={styles.checkMarkets} aria-label={sgText(locale, `Offer ${prefix.toUpperCase()} market`)}>
    {sgText(locale, Object.entries(labels).map(([market, label]) => {
      ...
      const loaded = typed === model.drafts.a.market || (model.mode === 'compare' && typed === model.drafts.b.market);
...
export function SingaporeCheckWorkspace({ locale = 'en', model }: Readonly<{ locale?: MarketLocale; model: SingaporeCheckRouteModel }>) {
  ...
  return <SingaporePage locale={locale} currentHref={marketHref(locale, "/sg/singapore/check/")}><div className={`${styles.checkWorkspace} ${toolSurface.surface}`} data-singapore-check-workspace="true">
    <header data-tool-header className={styles.checkHeader}><div><p className={styles.eyebrow}>{sgText(locale, "Singapore Check")}</p><h1>{sgText(locale, "Compare an asking price")}</h1></div><p>{sgText(locale, "Recent completed months only")}<br />{sgText(locale, "Minimum 5 comparable transactions")}</p></header>
    ...
    <section data-tool-layout className={styles.checkBody}><div data-tool-input className={styles.checkForm}><Form prefetch={false} action={marketHref(locale, "/sg/singapore/check/")} scroll={false}><Fragment key={JSON.stringify([model.mode, model.drafts])}><PassportFormContext /><input type="hidden" name="submitted" value="1" /><input type="hidden" name="mode" value={model.mode} /><MarketTabs locale={locale} prefix="a" model={model} /><OfferFields locale={locale} prefix="a" draft={model.drafts.a} catalog={model.catalogs[model.drafts.a.market]} />{model.mode === 'compare' ? <><MarketTabs locale={locale} prefix="b" model={model} /><OfferFields locale={locale} prefix="b" draft={model.drafts.b} catalog={model.catalogs[model.drafts.b.market]} /></> : null}<button className={styles.checkSubmit} type="submit" disabled={!available}>{sgText(locale, model.mode === 'compare' ? 'Compare offers' : model.drafts.a.market === 'hdb-rent' ? 'Compare an asking rent' : 'Compare an asking price')}</button></Fragment></Form></div><aside data-tool-result className={styles.checkResult} aria-label={sgText(locale, "Check result")}><ResultPanel locale={locale} model={model} /></aside></section>
