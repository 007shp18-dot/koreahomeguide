...
function OfferFields({ locale = 'en', prefix, draft, catalog }: Readonly<{ locale?: MarketLocale; prefix: 'a' | 'b'; draft: SingaporeCheckDraft; catalog: SingaporeCheckCatalog }>) {
  if (!catalog.available) return <fieldset className={styles.offerFields}>
    ...
    <p>{sgText(locale, "Individual transaction evidence for this price check is not available in the current release.")}</p>
    <Link href={marketHref(locale, draft.market === 'ura-private-sale' ? '/sg/singapore/explore/' : '/sg/singapore/explore/')}>{sgText(locale, "Explore published Singapore evidence")}</Link>
  </fieldset>;
  ...
  return <fieldset className={styles.offerFields} data-offer={prefix.toUpperCase()}>
    ...
    {field('Completed month', `${prefix}-month`, <select name={`${prefix}-month`} defaultValue={draft.month}><option value="">{sgText(locale, "Latest available")}</option>{options(locale, catalog.months, draft.month)}</select>)}
  </fieldset>;
}
...
function MarketTabs({ locale = 'en', prefix, model }: Readonly<{ locale?: MarketLocale; prefix: 'a' | 'b'; model: SingaporeCheckRouteModel }>) {
  ...
  return <nav className={styles.checkMarkets} aria-label={sgText(locale, `Offer ${prefix.toUpperCase()} market`)}>
    {sgText(locale, Object.entries(labels).map(([market, label]) => {
      ...
      return <Link key={market} href={marketHref(locale, `/sg/singapore/check/?${query}`)} aria-current={draft.market === market ? 'page' : undefined} data-evidence={available ? 'ready' : 'unavailable'}><strong>{sgText(locale, label)}</strong><span>{sgText(locale, available ? 'Evidence ready' : 'Evidence unavailable')}</span></Link>;
    }))}
  </nav>;
}
...
function ReadyResult({ locale = 'en', result, label = 'Offer' }: Readonly<{ locale?: MarketLocale; result: Extract<SingaporeCheckResult, { status: 'ready' }>; label?: string }>) {
  return <article className={styles.resultCard}><p className={styles.sectionLabel}>{sgText(locale, label)}{sgText(locale, " · ")}{sgText(locale, labels[result.market])}</p><strong className={styles.resultMedian}>{sgText(locale, money(result.distribution.median))}</strong><dl><div><dt>{sgText(locale, "P25–P75")}</dt><dd>{sgText(locale, money(result.distribution.p25))}{sgText(locale, "–")}{sgText(locale, money(result.distribution.p75))}</dd></div><div><dt>{sgText(locale, "Offer position")}</dt><dd>{sgText(locale, result.percentile)}{sgText(locale, "th percentile")}</dd></div><div><dt>{sgText(locale, "Comparable scope")}</dt><dd>{sgText(locale, result.scope.label)}</dd></div><div><dt>{sgText(locale, "Sample")}</dt><dd>{sgText(locale, result.sampleCount)}</dd></div><div><dt>{sgText(locale, "Completed window")}</dt><dd>{sgText(locale, result.window.from)}{sgText(locale, "–")}{sgText(locale, result.window.to)}</dd></div><div><dt>{sgText(locale, "Source")}</dt><dd>{sgText(locale, result.sourceIdentifier)}</dd></div></dl>{result.fallbackDisclosure === null ? null : <p>{sgText(locale, result.fallbackDisclosure)}</p>}</article>;
}
function OfferResult({ locale = 'en', result, label }: Readonly<{ locale?: MarketLocale; result: SingaporeCheckResult; label?: string }>) {
  ...
  if (result.status === 'insufficient') return <article className={styles.resultCard}><p className={styles.sectionLabel}>{sgText(locale, label)}</p><h3>{sgText(locale, "Insufficient recent evidence")}</h3><p>{sgText(locale, result.sampleCount)}{sgText(locale, " comparable records · minimum ")}{sgText(locale, result.minimumSample)}</p><p>{sgText(locale, result.window.from)}{sgText(locale, "–")}{sgText(locale, result.window.to)}{sgText(locale, "; the time window was not widened.")}</p></article>;
...
function ResultPanel({ locale = 'en', model }: Readonly<{ locale?: MarketLocale; model: SingaporeCheckRouteModel }>) {
  if (model.result.kind === 'empty') return <><p className={styles.sectionLabel}>{sgText(locale, "Result")}</p><h2>{sgText(locale, "Enter an offer to check its market position.")}</h2><p>{sgText(locale, "Results show median, P25–P75, percentile, sample, completed window, scope and source.")}</p></>;
