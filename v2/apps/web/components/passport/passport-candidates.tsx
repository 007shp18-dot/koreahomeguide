...
export function PassportCandidates({ market, locale, passportHref }: Readonly<{
  ...
  return <section className={styles.matchRow} aria-label={`${market.city} · ${copy.heading}`}>
    ...
    <p>{copy.note}</p>
    {market.matches.length > 0 ? <label className={styles.candidateSearch}><span>{locale === 'ko' ? '단지명·지역명 검색' : locale === 'zh-CN' ? '按名称或地区筛选' : 'Filter by name or area'}</span><input type="search" value={search} onChange={event => {setSearch(event.target.value); setPage(1);}} aria-label={`${market.city} · ${locale === 'ko' ? '후보 검색' : locale === 'zh-CN' ? '筛选候选' : 'Filter candidates'}`} /></label> : null}
    ...
    {market.matches.length === 0 ? <p>{market.scopes.length === 0 ? copy.missing : copy.none}</p> : <>
      <ul className={styles.candidateList}>{matches.slice((activePage - 1) * PAGE_SIZE, activePage * PAGE_SIZE).map(scope => {
        ...
        const href = passportCandidateHref(marketHref(locale, scope.href), passportHref);
        const scenario = createPropertyScenarioHref({locale,market:market.id,currency:market.currency,
