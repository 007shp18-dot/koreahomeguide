// Historical design-review fixture: keep its content and layout independent of the live feed.
import 'server-only';
import Link from 'next/link';
import { getPortfolioRecord } from '@/content/portfolio-manifest';
import type { SiteLocale } from '@/lib/navigation/site-navigation';
import defaultStyles from './editorial-growth-archived-home.module.css';
...
export function HomeAnalysis({ locale, classNames: styles = defaultStyles }: { locale: SiteLocale; classNames?: typeof defaultStyles }) {
  ...
  return <section className={`${styles.section} ${styles.analysis}`} aria-labelledby="home-analysis-title" data-home-region="analysis">
    ...
    <div className={styles.analysisGrid}>{articles.map(({ record, city }) => <article key={record.id}>
