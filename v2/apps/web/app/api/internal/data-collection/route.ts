...
export async function GET(request: Request) {
 ...
 try {
  ...
  const [sources,markets,quality] = await Promise.all([repository.status(),marketCollectionStatus(sql),dataQuality(sql)]);
  return reply({ sources, markets, quality, publication: 'Raw snapshots require review and structured evidence before publication.' });
