...
export default async function ChineseGuidePage({ params }: Props) {
  const guide = getPortfolioRecord('zh-CN', (await params).slug); if (guide?.type !== 'guide') notFound();
  return <EditorialGrowthPublicFrame locale="zh-CN" surface="content" activeSection="guides" currentHref={guide.canonicalHref}><NewsroomArticle article={guide} /><PublicEditorialJsonLd article={guide} /></EditorialGrowthPublicFrame>;
}
