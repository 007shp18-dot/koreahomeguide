...
export function generateStaticParams() { return listPortfolioRecords('zh-CN').filter(({ type }) => type === 'policy-update').map(({ slug }) => ({ slug })); }
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const article = getPortfolioRecord('zh-CN', (await params).slug); if (article?.type !== 'policy-update') notFound();
...
export default async function ChinesePolicyPage({ params }: Props) {
  const article = getPortfolioRecord('zh-CN', (await params).slug); if (article?.type !== 'policy-update') notFound();
