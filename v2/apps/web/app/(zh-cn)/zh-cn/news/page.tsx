...
export const metadata: Metadata = indexableMetadata({ path: '/zh-cn/news/', title: '房地产政策、市场与数据故事 | signedprice', description: '首尔与新加坡的已核验中文政策更新、市场简报和数据故事。', languageAlternates: { en: '/news/', ko: '/ko/news/', 'zh-Hans': '/zh-cn/news/' }, locale: 'zh_CN' });

export default async function ChineseNewsPage({ searchParams }: Readonly<{ searchParams?: Promise<{ type?: string | string[] }> }> = {}) {
  const analysisOnly = (await searchParams)?.type === 'analysis';
  const records = listPortfolioRecords('zh-CN').filter(({ type }) => analysisOnly ? type === 'data-story' || type === 'market-brief' : type !== 'guide');
