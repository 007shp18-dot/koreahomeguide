...
export const decisionTopics = {
  ...
  fees: {
    ...
    investor: decisionText('임대료에서 소유자 부담 관리비·수선·공실 비용을 뺀 뒤에도 매입가를 설명할 수 있어야 합니다. 광고 임대료만으로 수익률을 계산하면 빠지는 비용이 생깁니다.', 'The purchase case needs to survive owner-paid charges, repairs and vacancy. An advertised rent alone leaves too much out of a yield calculation.', '扣除业主承担的费用、维修及空置成本后，购入价仍应有合理依据。仅用挂牌租金计算收益率，会漏掉重要支出。'),
