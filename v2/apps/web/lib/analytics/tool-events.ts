...
export type ToolId = 'tools-hub'|'passport'|'single-quote'|'offer-compare'|'rent-check'|'property-scenario'|'singapore-check';
...
export function createToolEvent(event: ToolEvent,input: ToolDimensions & Readonly<Record<string,unknown>>) {
 ...
 || !['tools-hub','passport','single-quote','offer-compare','rent-check','property-scenario','singapore-check'].includes(input.tool)) throw new TypeError('Invalid tool event');
