...
const markets = new Set<EditorialMarket>(['kr-seoul', 'sg-singapore', 'ae-dubai', 'jp-tokyo']);
...
export function createEditorialEvent(event: EditorialEvent, input: Readonly<Record<string, unknown>> & Readonly<{
  ...
  if (input.locale !== 'en' && input.locale !== 'zh-CN' && input.locale !== 'ko') throw new TypeError('Invalid editorial locale.');
