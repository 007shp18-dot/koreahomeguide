...
export async function marketCollectionStatus(sql: SqlPort): Promise<MarketCollectionStatus[]> {
  ...
  ) SELECT latest.*, success.completed_at AS last_success_at, success.source_as_of AS last_success_source_as_of, previous.received AS previous_received,
  ...
  return [...MARKET_REFRESH_JOBS, 'jp-tokyo-sale'].map(job => {
    const row = rows.find(value => value.job === job);
    return { job, enabled: job === 'jp-tokyo-sale' ? process.env.SIGNEDPRICE_JAPAN_REFRESH_ENABLED === 'true' : validConfiguration && enabled.includes(job), state: row?.state == null ? null : String(row.state),
      lastAttemptAt: instant(row?.started_at), lastSuccessAt: instant(row?.last_success_at), sourceAsOf: instant(row?.last_success_source_as_of),
