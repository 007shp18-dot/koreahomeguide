...
export function createCollectionRepository(sql: SqlPort) {
 return {
  async status(): Promise<CollectionStatus[]> {
   ...
   return COLLECTION_SOURCES.map((source) => { const row = rows.find((r) => r.source_id === source.id); return { sourceId: source.id, name: source.name, market: source.market, category: source.category, url: source.url, mode: source.mode, intervalDays: source.intervalDays, limitation: source.limitation, lastSuccessAt: time(row?.last_success_at), lastAttemptAt: time(row?.last_attempt_at), nextDueAt: time(row?.next_due_at), lastPublishedAt: source.id==='sg-hdb-buildings'?time(hdb?.published_at):null, consecutiveFailures: Number(row?.consecutive_failures ?? 0), lastError: row?.last_error ? String(row.last_error) : null, anomaly: row?.anomaly ? String(row.anomaly) : null, newCount: Number(row?.new_count ?? 0), changedCount: Number(row?.changed_count ?? 0), pendingCount: Number(row?.pending_count ?? 0), latestSnapshotId: row?.latest_snapshot_id ? String(row.latest_snapshot_id) : null, probe: (probes.find((p)=>p.source_id===source.id)?.result as Record<string,unknown>) ?? null, pendingCandidateCount: source.id==='sg-hdb-buildings'?Number(hdb?.pending??0):0, recordCount: source.id==='sg-hdb-buildings'?Number(hdb?.records??0):0 }; });
  },
