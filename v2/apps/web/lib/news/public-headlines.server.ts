...
export async function readPublicHeadlines() {
  ...
  const [stored, overrides] = await Promise.all([loadPersistedNewsItems(1500), publicationOverrides()]);
  if (overrides === null || (configured && stored === null)) return null;
  const now = Date.now();
  ...
  return mergeReviewedHeadlines(stored ?? [], [...portfolioHeadlines(files, now), ...checked], overrides.excluded, now);
}
...
const cachedHeadlines = unstable_cache(async () => {
  ...
  return items;
}, ['public-reviewed-headlines-v1'], { revalidate: 900 });
