...
import { researchPriorities, type ResearchPriority } from './research-priority.server';
...
export async function researchDashboard(sql: SqlPort, market: string, tool: string): Promise<ResearchDashboard> {
  ...
  const [groups, distributions, recent, priorities, maintenance] = await Promise.all([
    ...
    researchPriorities(sql, market, tool),
    sql.query("SELECT last_aggregated_at FROM tool_research_maintenance WHERE id = 'daily'"),
  ]);
  ...
  return { total: mapped.reduce((sum, r) => sum + r.count, 0), groups: mapped,
    priorities, countedAt: new Date().toISOString(),
    lastAggregatedAt: maintenance[0]?.last_aggregated_at ? new Date(String(maintenance[0].last_aggregated_at)).toISOString() : null,
