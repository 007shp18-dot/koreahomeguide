...
export function createPublicAreaSummaryJobPostHandler(
  ...
  return async function post(request: Request): Promise<Response> {
    ...
    try {
      ...
      const districtNSum = finalization.districtSummaries.reduce(
        (sum, summary) => sum + summary.n,
        0,
      );
      if (
        finalization.completedCoordinates !== 700 ||
        finalization.districtSummaries.length !== 25 ||
        finalization.citySummary.n !== districtNSum ||
      ...
      if (
        typeof built.serialized !== 'string' ||
        !/^[0-9a-f]{64}$/.test(built.sha256)
      ) {
        return error('internal_error', 500);
      }
      return json({
        ...
        cityN: finalization.citySummary.n,
        districtCount: finalization.districtSummaries.length,
        districtNSum,
