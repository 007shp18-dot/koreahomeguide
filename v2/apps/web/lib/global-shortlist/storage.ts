...
export function parseGlobalSaved(raw: string): GlobalSaved {
  const empty: GlobalSaved = { version: 1, filters: { singapore: defaults('singapore'), dubai: defaults('dubai') }, places: [] };
  try {
    ...
    for (const market of ['singapore', 'dubai'] as const) if (validBudgetFilters(value.filters[market], market)) empty.filters[market] = value.filters[market];
    const keys = new Set<string>();
    empty.places = value.places.filter(p => {
      if (!p || !['singapore', 'dubai'].includes(p.market) || typeof p.key !== 'string' || !/^[a-zA-Z0-9_-]{1,100}\/[a-zA-Z0-9_-]{1,160}$/.test(p.key) || typeof p.name !== 'string' || p.name.length > 200 || !/^[a-f0-9]{64}$/.test(p.signature) || typeof p.checkedAt !== 'string' || !Number.isFinite(Date.parse(p.checkedAt))) return false;
