export type OverseasMarket = 'singapore' | 'dubai' | 'tokyo';
export type BudgetFilters = { budget: number; minArea: number; maxArea: number; region: string; housing: string; completion: string };
export const defaults = (market: OverseasMarket): BudgetFilters => ({ budget: market === 'tokyo' ? 100_000_000 : market === 'singapore' ? 1_500_000 : 1_000_000, minArea: 80, maxArea: 100, region: 'all', housing: market === 'singapore' ? 'condominium' : 'apartment', completion: 'ready' });
export function validBudgetFilters(f: BudgetFilters, market: OverseasMarket): boolean {
  return !!f && Number.isFinite(f.budget) && f.budget > 0 && f.budget <= 500_000_000
    ...
    && (market === 'singapore' ? ['condominium', 'apartment', 'executive_condominium'].includes(f.housing) : (market === 'tokyo' ? ['apartment'] : ['apartment', 'villa']).includes(f.housing))
    && (market === 'tokyo' ? f.completion === 'ready' : ['ready', 'off-plan'].includes(f.completion));
}
...
export const cityPaths = { seoul: '/kr/seoul/shortlist/', singapore: '/sg/singapore/shortlist/', dubai: '/ae/dubai/shortlist/', tokyo: '/jp/tokyo/shortlist/' } as const;
