...
import { reviseNeighbourhood } from './revisions/neighbourhoods';
...
export const NEIGHBOURHOOD_STORIES: readonly NeighbourhoodStory[] = stories.map(story => reviseNeighbourhood(story, 'en'));
...
function localize(story: NeighbourhoodStory, locale: 'en' | 'ko'): NeighbourhoodStory {
  ...
  if (!translation) return reviseNeighbourhood(story, locale);
  const { captions, sourceLabels, sections, ...copy } = translation;
  return reviseNeighbourhood({ ...story, ...copy,
