...
export const CITY_BUDGET_COMPARISONS: readonly EditorialPortfolioRecord[] = (['tokyo'] as BuyingCity[]).flatMap(city => (['en','ko','zh-CN'] as const).map(locale=>{
  ...
  const title=t(['Tokyo apartment budgets: JPY 30M, 50M and 100M across five wards','도쿄 아파트 예산 비교: 3천만·5천만·1억 엔으로 본 다섯 구','东京公寓预算比较：3,000万、5,000万与1亿日元的五区成交']), slug=comparisonSlug(city);
