...
export const REGIONAL_RESOURCES: Record<StoryCity, { name: StoryText; intro: StoryText; checklist: Item[]; glossary: Item[]; related: string }> = {
  seoul: {
    ...
    checklist: [
      ...
      item('Match the home and the contracting party', '매물과 계약 상대방 대조', 'Check the exact address and unit against a current 등기사항증명서 (property register), commonly called 등기부등본. Confirm who signs and why the named account receives the payment.', '정확한 주소·호실을 최신 소유권 자료와 대조하고 서명자와 입금 계좌 명의의 관계를 확인하세요.'),
      ...
      item('Arrange 전입신고 and 확정일자', '전입신고·확정일자 준비', 'Take possession, complete the applicable 전입신고 (move-in/resident registration report at the local community service centre) and obtain 확정일자 (official fixed-date stamp on the lease) promptly, preferably on move-in day. Possession and registration establish opposability from the following day; preferential repayment also requires the fixed date. Foreign residents should confirm the equivalent foreigner-registration and 체류지 변경신고 process.', '가능하면 입주 당일 주택을 인도받고 본인에게 적용되는 전입신고와 확정일자를 갖추세요. 주택 인도와 주민등록을 마친 다음 날 대항력이 발생하며, 우선변제권에는 이 요건들과 확정일자가 필요합니다. 외국인은 외국인등록·체류지 변경신고 절차를 확인하세요.'),
      item('Check 전세보증금 반환보증 eligibility', '전세보증금 반환보증 가입 조건 확인', 'Check deposit-return protection with HUG (전세보증금반환보증), HF (일반전세지킴보증) or SGI (전세금보장신용보험) before relying on it. Eligibility, application deadlines and coverage differ by institution and product.', 'HUG 전세보증금반환보증, HF 일반전세지킴보증, SGI 전세금보장신용보험의 가입 가능 여부를 확인하세요. 가입 조건·신청 기한·보장 범위는 기관과 상품별로 다릅니다.'),
