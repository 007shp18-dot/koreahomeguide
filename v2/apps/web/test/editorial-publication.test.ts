...
describe('editorial publication worker',()=>{
 ...
 it('scopes immediate publication with a bound queue ID',async()=>{const id='12345678-1234-1234-1234-123456789012';await publishDueEditorial(id);expect(mocks.query.mock.calls[0]?.[0]).toContain('id=$1::uuid');expect(mocks.query.mock.calls[0]?.[1]).toEqual([id]);});
