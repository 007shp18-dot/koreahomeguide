...
describe('editorial admin revisions',()=>{
 it('finds a queued dashboard article outside the latest 100 without writing',async()=>{const r=await GET(new Request('https://www.signedprice.com/api/internal/editorial-operations/?queueSlug=old-draft'));expect(r.status).toBe(200);expect(mocks.query).toHaveBeenCalledExactlyOnceWith('SELECT * FROM editorial_publication_queue WHERE slug=$1',['old-draft']);expect(mocks.publish).not.toHaveBeenCalled();});
