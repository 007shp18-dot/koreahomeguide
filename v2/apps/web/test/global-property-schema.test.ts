...
describe('global property database migration', () => {
  it('loads the global core after the existing persistent-content migrations', async () => {
    ...
    expect(bundles.map(({ name }) => name)).toEqual([
      ...
      '0020_property_evidence_pool.sql',
      '0021_data_collection.sql',
      ...
      '0026_editorial_publication_queue.sql',
      '0027_singapore_explore_publication.sql',
      '0028_tool_usage_daily.sql',
      '0029_japan_backfill.sql',
      '0030_tokyo_editorial_market.sql',
      '0031_editorial_image_uploads.sql',
      '0032_community_questions.sql',
      '0033_purchase_enquiries.sql',
    ]);
