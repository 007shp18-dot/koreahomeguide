...
describe('launch editorial portfolio', () => {
  ...
  it('publishes the 88-item portfolio including four Chinese investment and monthly translations', () => {
    expect(EDITORIAL_PORTFOLIO).toHaveLength(88);
    expect(Object.isFrozen(EDITORIAL_PORTFOLIO)).toBe(true);
    expect(EDITORIAL_PORTFOLIO.filter(({ locale }) => locale === 'en')).toHaveLength(38);
    expect(EDITORIAL_PORTFOLIO.filter(({ locale }) => locale === 'ko')).toHaveLength(38);
    expect(EDITORIAL_PORTFOLIO.filter(({ locale }) => locale === 'zh-CN')).toHaveLength(12);
  ...
  it('keeps every published claim attached to review, evidence and an internal next step', () => {
    ...
    expect(new Set(EDITORIAL_PORTFOLIO.map(({ id }) => id)).size).toBe(88);
    expect(new Set(EDITORIAL_PORTFOLIO.map(({ canonicalHref }) => canonicalHref)).size).toBe(88);
