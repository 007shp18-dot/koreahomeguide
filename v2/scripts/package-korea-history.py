...
import csv,gzip,hashlib,json,os,zipfile,shutil,xml.etree.ElementTree as ET
from pathlib import Path
ROOT=Path(os.environ.get('SIGNEDPRICE_HISTORY_OUTPUT', 'korea-history-2020-latest')).resolve()
...
assert {(c['lawdCd'],c['dealYmd']) for c in manifest['coordinates']}=={(d,m) for d in districts for m in months}
...
with gzip.open(output,'rb') as csv_source:manifest['csvSha256']=hashlib.file_digest(csv_source,'sha256').hexdigest()
...
with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_STORED) as z:
 for name in ['README.txt','manifest.json']:z.write(ROOT/name,name)
 info=zipfile.ZipInfo(output.name.removesuffix('.gz'));info.compress_type=zipfile.ZIP_DEFLATED
 with z.open(info,'w') as dest,gzip.open(output,'rb') as src:shutil.copyfileobj(src,dest)
