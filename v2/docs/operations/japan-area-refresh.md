...
6. Enable `SIGNEDPRICE_JAPAN_REFRESH_ENABLED=true` only after this first flow succeeds and the existing CRON_SECRET is available. The added Vercel cron runs Wednesday 02:40 UTC and fetches one Minato quarter per invocation. On an authenticated, enabled GET with no explicit scope, it first checks the uncached canonical Minato 2025 Q4 publication using the same database port. If absent, that one scope is collected to seed the default public page. Once it exists, the original rotation over eight completed quarters applies. A failed read stops collection rather than treating failure as absence. This also allows the dashboard Cron Run action to perform the initial bootstrap without exporting secrets. This deliberately modest initial scope revisits corrections, accommodates publication lag and avoids source bursts. Other wards can be collected with explicit operator scopes; they are not silently enabled in the scheduler.
...
- GET `/api/internal/japan-refresh/` with existing `CRON_SECRET` requires the enabled gate and chooses the canonical first-publication bootstrap when absent, otherwise one scheduled scope.
