...
Second CI run 34022594609: verify passed; browser 421 passed, 4 failed, 21 existing conditional skips. All four failures target the removed Seoul-only “Compare offers” link on global Prices. Updated these flows to keep the Prices → Seoul Explore round trip and reach Check through the actual Seoul market navigation, including touch-target and keyboard checks. Mobile language overlap, canonical/sitemap checks, all changed visual baselines, Singapore search/chart flows and Dubai research/calculator/news checks passed. Product code remains unchanged from 1bddb18; only the remaining stale browser flows and this record change in the final verification commit.
