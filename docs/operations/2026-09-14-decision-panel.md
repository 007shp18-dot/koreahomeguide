...
The first hosted browser pass completed 783 scenarios successfully, with 39 fixture-dependent skips and three failures. The failures identified a native-dialog keyboard boundary and a retry scenario pointing to a Seoul identity absent from the test installation. Desktop layout, report scrolling, all three locales, close/expand handling and the independent Seoul map checks passed. The desktop and mobile screenshots were inspected against the existing site design.

The isolated Helio retry then exposed two interactive sessions during the whole-page Suspense handoff. Seoul's hydrating fallbacks now render the main evidence without an active decision session; the hydrated detail creates the single report. Regression assertions retain the full evidence comparison and verify one workspace, the original main element and the selected perspective after retry. Mobile focus and retry passed the preceding isolated run.
