...
The browser trace also identified a real prefetched 404: the district detail language switch targeted a Korean district-detail route that does not exist. It now links to the available Korean Explore route with the district query preserved. A rendered-page regression covers the destination.

Browser contract reconciliation retains coverage rather than removing it: six existing Singapore/Dubai intent pages move from obsolete 404 expectations into the noindex route checks; district selection uses the current chip navigation; rankings separate published rows from visible withheld rows; publisher/source checks reject the unsupported review credits. Missing visual baselines still require inspection of the current rendered images.

Further validation: aligned rent-check controls to 48px (44px housing targets inside borders), fixed the shared Explore rail at 420px, and kept the mobile brand row at 44px even without a language switch. Removed unsupported Reviewed claims from guide indexes. Reviewed and recorded 16 current fixture screenshots; these establish regression baselines, not completion of the broader redesign. Legacy guide URLs are verified as redirects, followed by keyboard navigation to a current source-backed guide.

Browser checkpoint 20d8ca7: 394 passed, 21 skipped, 9 failed. All 16 visual baselines passed. Follow-up fixes address map-control overlap on small screens, district chip targets, logical min-block-size overriding rent input height, and stale sale-detail/home-alternate expectations. English and Chinese legacy desk labels were also removed; the eight home/content baseline images need refreshed captures for those intentional copy changes.
