...
**후속 확장 시장:** Dubai, Japan (서울·싱가포르 완료 후)

## 2026-09-06 사용자 확정 사항 (이전 범위보다 우선)

최종 사업 목표는 리서치 유입에서 해외 투자 상담, 현지 중개·매물 연결, 계약·매수 지원, 임대·관리·매각 지원까지 이어지는 것이다. 직접 중개와 현지 파트너 운영 방식은 국가별로 후속 설계한다. 현재 가격 증거에 해당하는 건물과 실제 중개 가능한 매물은 별도 개념으로 유지한다.

순서는 서울·싱가포르 Prices/Explore 검색·목록·지도와 기존 UI·콘텐츠 개편 완료 → 검증·운영 반영 → 두바이·일본 확장이다. 일본의 첫 도시는 아직 확정하지 않았다. England & Wales는 이번 승인 로드맵에 포함하지 않는다. 기존 문서의 후보 시장 언급은 과거 조사 후보 기록이다.

전체 톤은 유지한다. 기존 네이비·화이트, 본문 16px 이상, UI 14px 이상, 보조 정보 12px 이상, 주요 조작 44px 이상(검색 입력·버튼 48px), 8/12/16/24/32px 간격, 8/12px 반경을 재사용한다. 색상은 공통 토큰을 사용하고 상태·선택은 글자와 ARIA도 제공한다. 검색→지역→필터→결과 목록·지도→상세 순서를 공유하되 도시별 데이터 구조는 보존한다. 기능마다 별도 테마나 장식 박스를 만들지 않는다.
