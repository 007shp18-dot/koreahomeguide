...
- Consolidate 12 translated same-budget records into four existing city guides. Preserve expanded Tokyo ward data; redirect retired URLs. Separate budget navigation from general Insights and remove duplicate Investment tab.
- Rename global practical-guide navigation, remove duplicate city comparison links.
- Compact home budget controls and transaction results into rows; replace duplicated next-step cards with action links.
- Default Seoul detail to the largest publishable sale size cohort, with 60–85 tie preference and explicit selection preserved. Use complete aggregates, not recent preview rows.
...
## Statistical corrections agreed with owner

Use matched cohort medians for neighbourhood comparison. Use interquartile dispersion, never infer a floor premium from two extrema. Suppress small new/renewal subgroups separately. Missing or mismatched evidence means omitted interpretation. Current legal/fee inputs must be checked against official sources, not copied uncritically from sample JSON.
