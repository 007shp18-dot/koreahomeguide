...
test('cold-start homepage guide section opens the real guide pages in both languages', () => {
  ...
  assert.equal((enGrid.match(/class="home-guide-row"/g)||[]).length,4);
  assert.equal((zhGrid.match(/class="home-guide-row"/g)||[]).length,4);
  for(const href of ['/guides/rent-apartment-korea-foreigner/','/guides/wolse-vs-jeonse/','/guides/before-you-sign/','/guides/seoul-officetel-rent/']) assert.match(enGrid,new RegExp(`href="${href.replaceAll('/','\\/')}"`));
  for(const href of ['/zh/guides/rent-apartment-korea-foreigner/','/zh/guides/wolse-vs-jeonse/','/zh/guides/before-you-sign/','/zh/guides/seoul-officetel-rent/']) assert.match(zhGrid,new RegExp(`href="${href.replaceAll('/','\\/')}"`));
