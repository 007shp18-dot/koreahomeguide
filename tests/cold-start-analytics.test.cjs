...
test('both canonical Rent Check runtimes dispatch the shared result event', () => {
...
test('both canonical Rent Check runtimes structurally preserve safe tool-view and failure analytics', () => {
  for (const file of files) {
    const source = fs.readFileSync(file, 'utf8');
