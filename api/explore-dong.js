...
function createHandler(providerFactory = options => createKoreaHousingProvider(options)) {
  return async function handler(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ city:'seoul', districtCode:areaCode, districtName:SEOUL_DISTRICTS[areaCode], propertyType, dong, summary, buildings });
    } catch (err) {
      logApiError('explore-dong', err, { lawdCd:areaCode, type:propertyType, dong });
