...
function createHandler(providerFactory = options => createKoreaHousingProvider(options)) {
  return async function handler(req, res) {
    ...
    try {
      ...
      const detail = await provider.getBuildingDetail({ areaCode, propertyType, buildingKey, months:6 });
      if (!detail) return res.status(404).json({ error:'Building not found in the selected recent transaction period.' });
      res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate=86400');
      return res.status(200).json({ city:'seoul', districtCode:areaCode, districtName:SEOUL_DISTRICTS[areaCode], propertyType, ...detail });
    } catch (err) {
      logApiError('explore-building', err, { lawdCd:areaCode, type:propertyType, buildingKey });
