...
function primaryLocationLabel(kind, value, lang) {
  const locale = isZh(lang) ? 'zh-CN' : 'en';
  if (kind === 'district') return KHGLocations.districtLabel(value, locale, { includeKorean:false });
  if (kind === 'property') return KHGLocations.propertyTypeLabel(value, locale, { includeKorean:false });
  return KHGLocations.dongLabel(value, locale, { includeKorean:false });
}
...
function labeledCell(label, html, className = '') {
  return `<td${className ? ` class="${escapeHtml(className)}"` : ''} data-label="${escapeHtml(label)}">${html}</td>`;
}
...
function recentContractsTable(items, lang, rates, withBuilding = true) {
  ...
  const labels = zh
    ? { building:'建筑', type:'类型', size:'面积', deposit:'押金', rent:'月租', date:'签约日期' }
    : { building:'Building', type:'Type', size:'Size', deposit:'Deposit', rent:'Monthly rent', date:'Contract date' };
  const headers = [
    ...(withBuilding ? [labels.building] : []), labels.type, labels.size, labels.deposit, labels.rent, labels.date
  ].map(label => `<th>${escapeHtml(label)}</th>`).join('');
  const body = rows.map(item => {
    let building = '';
    if (withBuilding) {
      const name = getBuildingNameDisplay(item.building || '-', lang);
      building = labeledCell(labels.building, `<strong>${escapeHtml(name.primary)}</strong>${name.secondary ? `<small class="seo-building-official">${escapeHtml(name.secondary)}</small>` : ''}`, 'seo-building-cell');
    }
    const rent = Number(item.monthlyRentWon) === 0
      ? `<span class="seo-money">${zh ? '全租式 · ₩0' : 'Jeonse-style · ₩0'}</span>`
      : moneyHtml(item.monthlyRentWon, lang, rates);
    return `<tr>${building}${labeledCell(labels.type, escapeHtml(contractTypeLabel(item.contractType, lang)))}${labeledCell(labels.size, escapeHtml(areaText(item.areaSqm)))}${labeledCell(labels.deposit, moneyHtml(item.depositWon, lang, rates))}${labeledCell(labels.rent, rent)}${labeledCell(labels.date, escapeHtml(KHGDate.formatDate(item.contractDate, zh ? 'zh-CN' : 'en-US')))}</tr>`;
  }).join('');
  return `<div class="seo-table-wrap"><table class="seo-table"><thead><tr>${headers}</tr></thead><tbody>${body}</tbody></table></div>`;
}
...
function recentSalesTable(items, lang, rates) {
  ...
  if (!rows.length) return '';
  const labels = zh
    ? { size:'面积', price:'成交价', floor:'楼层', date:'成交日期' }
    : { size:'Size', price:'Sale price', floor:'Floor', date:'Contract date' };
  const headers = [labels.size, labels.price, labels.floor, labels.date].map(label => `<th>${escapeHtml(label)}</th>`).join('');
  const body = rows.map(row => `<tr>${labeledCell(labels.size, escapeHtml(areaText(row.areaSqm)))}${labeledCell(labels.price, moneyHtml(row.dealAmountWon, lang, rates))}${labeledCell(labels.floor, row.floor == null || !Number.isFinite(Number(row.floor)) ? '—' : escapeHtml(String(row.floor)))}${labeledCell(labels.date, escapeHtml(KHGDate.formatDate(row.contractDate, zh ? 'zh-CN' : 'en-US')))}</tr>`).join('');
  return `<div class="seo-table-wrap"><table class="seo-table"><thead><tr>${headers}</tr></thead><tbody>${body}</tbody></table></div>`;
}
...
function renderDongPage({ lang = 'en', areaCode, districtName, dong, propertyType, summary, buildings = [], fxRates = {} }) {
  ...
  const dongPrimary = primaryLocationLabel('dong', dong, lang);
  const districtPrimary = primaryLocationLabel('district', areaCode, lang);
  const propertyPrimary = primaryLocationLabel('property', propertyType, lang);
  const visibleTitle = zh
    ? `${dongPrimary} ${propertyPrimary}租金`
    : `${propertyPrimary} rents in ${dongPrimary}`;
